...
RegisterNetEvent('ctrp-lapraces:client:StartRaceEditor')
AddEventHandler('ctrp-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        CTRPFW.Functions.Notify('Your already making a race.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        CTRPFW.Functions.Notify('You have not placed a checkpoint ..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        CTRPFW.Functions.Notify('The minimum of checkpoints is 10', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        CTRPFW.Functions.Notify('You cant go higher than 15')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        CTRPFW.Functions.Notify('You cant go lower than 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    CTRPFW.Functions.Notify('Press again on [9] to confirm', 'error', 5000)
                else
                    ...
                    CTRPFW.Functions.Notify('Race-editor Canceled!', 'error')
...
function SaveRace()
    ...
    TriggerServerEvent('ctrp-lapraces:server:SaveRace', CreatorData)

    CTRPFW.Functions.Notify('Race: '..CreatorData.RaceName..' is saved!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            CTRPFW.Functions.Notify('You cant go to fast..', 'error')
        end
    else
        CTRPFW.Functions.Notify('You cant go to fast..', 'error')
    end
...
RegisterNetEvent('ctrp-lapraces:client:UpdateRaceRacerData')
AddEventHandler('ctrp-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('ctrp-lapraces:client:JoinRace')
AddEventHandler('ctrp-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('ctrp-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        CTRPFW.Functions.Notify('Your already in a race..', 'error')
    end
end)

RegisterNetEvent('ctrp-lapraces:client:LeaveRace')
AddEventHandler('ctrp-lapraces:client:LeaveRace', function(data)
    CTRPFW.Functions.Notify('You have left the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('ctrp-lapraces:client:RaceCountdown')
AddEventHandler('ctrp-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('ctrp-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    CTRPFW.Functions.Notify('The race will start in 10 seconds', 'error', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    CTRPFW.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            CTRPFW.Functions.Notify('GO!', 'success', 1000)
        ...
        CTRPFW.Functions.Notify('You are currently notin a race..', 'error')
    end
end)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('ctrp-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('ctrp-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('ctrp-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        CTRPFW.Functions.Notify('Race finished '..SecondsToClock(CurrentRaceData.TotalTime)..', Whit best lap as: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        CTRPFW.Functions.Notify('Race finished '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('ctrp-lapraces:client:PlayerFinishs')
AddEventHandler('ctrp-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            CTRPFW.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' Finished on place : '..Place, 'error', 3500)
        end
    end
end)
...
RegisterNetEvent('ctrp-lapraces:client:WaitingDistanceCheck')
AddEventHandler('ctrp-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            CTRPFW.Functions.Notify('Go back to the start in 10 seconds: '..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('ctrp-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)

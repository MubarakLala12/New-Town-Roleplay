...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(CTRP.Phone.Data.PlayerData.citizenid, RaceData.RaceData.Racers)
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "The race has already started..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            ...
            var InRace = IsInRace(CTRP.Phone.Data.PlayerData.citizenid, race.RaceData.Racers);
            var Creator = IsCreator(CTRP.Phone.Data.PlayerData.citizenid, race);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('https://ctrp-phone_new/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('https://ctrp-phone_new/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('https://ctrp-phone_new/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('https://ctrp-phone_new/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
                            ...
                            CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are in an editor..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are already in a race..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('https://ctrp-phone_new/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('https://ctrp-phone_new/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://ctrp-phone_new/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('https://ctrp-phone_new/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('https://ctrp-phone_new/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
            ...
            if (!data.IsBusy) {
                $.post('https://ctrp-phone_new/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're in a race..", "#1DA1F2");
                    }
                });
            } else {
                CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are already making a Track..", "#1DA1F2");
            }
        } else {
            CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have no rights to create Race Track's..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        $.post('https://ctrp-phone_new/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('https://ctrp-phone_new/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
                    ...
                    CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "This name is not available..", "#1DA1F2");
                }
            } else {
                CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have no rights to create Race Track's..", "#1DA1F2");
            }
        ...
        CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You must enter a Track Name..", "#1DA1F2");
    }
});
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('https://ctrp-phone_new/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('https://ctrp-phone_new/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('https://ctrp-phone_new/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('https://ctrp-phone_new/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
                                    ...
                                    CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Races cannot be created at the moment..", "#1DA1F2");
                                }
                            });
                        } else {
                            CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Enter a number of rounds..", "#1DA1F2");
                        }
                    } else {
                        CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have not chosen a Track..", "#1DA1F2");
                    }
                }
            })
        } else {
            CTRP.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You already have a race active..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('https://ctrp-phone_new/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {

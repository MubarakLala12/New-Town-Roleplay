...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Wallet: "+CryptoData.Portfolio+" Hydracoin('s)");
...
function UpdateCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Portefeuille: "+CryptoData.Portfolio+" Hydracoin('s)");
    $(".crypto-walletid").html(CryptoData.WalletId);
    $(".cryptotab-course-list").html("");
    if (CryptoData.History.length > 0) {
        CryptoData.History = CryptoData.History.reverse();
...
function RefreshCryptoTransactions(data) {
    ...
    if (data.CryptoTransactions.length > 0) {
        ...
        $.each(data.CryptoTransactions, function(i, transaction){
            ...
            if (transaction.TransactionTitle == "Depreciation") {
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (RS.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://ctrp-phone_new/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    RS.Phone.Notifications.Add("fas fa-university", "HBank", "There is &dollar; "+Price+" written off!", "#badc58", 2500);
                } else {
                    RS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You do not have enough money..", "#badc58", 1500);
                }
            });
        } else {
            RS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You do not have enough money..", "#badc58", 1500);
        ...
        RS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill in everything!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://ctrp-phone_new/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    RS.Phone.Data.PlayerData.money.bank = parseInt(RS.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    RS.Phone.Notifications.Add("fas fa-university", "HBank", "There is &dollar; "+Price+" credited!", "#badc58", 2500);
                } else {
                    RS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Hydracoin('s)..", "#badc58", 1500);
                }
            });
        } else {
            RS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Hydracoin('s)..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://ctrp-phone_new/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        RS.Phone.Notifications.Add("fas fa-university", "Crypto", "This Wallet ID does not exist!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        RS.Phone.Notifications.Add("fas fa-university", "Crypto", "You have"+Coins+" transferred to "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                RS.Phone.Notifications.Add("fas fa-university", "Crypto", "You cannot transfer to yourself..", "#badc58", 2500);
            }
        } else {
            RS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Hydracoin('s)..", "#badc58", 1500);
        }

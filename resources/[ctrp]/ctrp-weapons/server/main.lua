...
CTRPFW.Functions.CreateCallback("weapons:server:GetConfig", function(source, cb)
...
--     CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `playerammo` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
...
CTRPFW.Functions.CreateCallback("weapon:server:GetWeaponAmmo", function(source, cb, WeaponData)
...
CTRPFW.Functions.CreateUseableItem("pistol_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_PISTOL", 12, item)
end)

CTRPFW.Functions.CreateUseableItem("rifle_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_RIFLE", 30, item)
end)

CTRPFW.Functions.CreateUseableItem("smg_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SMG", 20, item)
end)

CTRPFW.Functions.CreateUseableItem("shotgun_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SHOTGUN", 10, item)
end)

CTRPFW.Functions.CreateUseableItem("mg_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_MG", 30, item)
end)

CTRPFW.Functions.CreateUseableItem("sniper_ammo", function(source, item)
...
--     local src = source
--     local Player = CTRPFW.Functions.GetPlayer(src)
--     local WeaponData = CTRPFW.Shared.Weapons[GetHashKey(data.name)]
...
--                 TriggerClientEvent('CTRPFW:Notify', src, "Your weapon is broken, you must fix this weapon before you can use it again.", "error")
--             end
--         else
...
CTRPFW.Commands.Add("repairweapon", "Fix weapon command for staff", {{name="hp", help="HP of your weapon"}}, true, function(source, args)
...
CTRPFW.Functions.CreateCallback("weapons:server:RepairWeapon", function(source, cb, RepairPoint, data)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.items[data.slot] ~= nil then
        if Player.PlayerData.items[data.slot].info.quality ~= nil then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCotsts[WeaponClass]) then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, CTRPFW.Shared.Items[data.name], "remove")
                    ...
                    SetTimeout(Timeout, function()
                        ...
                        TriggerClientEvent('weapons:client:SyncRepairShops', -1, Config.WeaponRepairPoints[RepairPoint], RepairPoint)
                        TriggerEvent('ctrp-phone:server:sendNewMailToOffline', Player.PlayerData.citizenid, {
                            sender = "Virus",
                            subject = "Repair",
                            message = "Yes "..WeaponData.label.." is repaired! You can come and pick it up at the fixed location. <br> <br> Peace out"
                        })
                ...
                TriggerClientEvent("CTRPFW:Notify", src, "This weapon has not been damaged.", "error")
                cb(false)
            end
        else
            TriggerClientEvent("CTRPFW:Notify", src, "This weapon has not been damaged.", "error")
        ...
        TriggerClientEvent('CTRPFW:Notify', src, "You didnt have a weapon in your hand..", "error")
...
AddEventHandler("weapons:server:TakeBackWeapon", function(k, data)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, CTRPFW.Shared.Items[itemdata.name], "add")
    Config.WeaponRepairPoints[k].IsRepairing = false
    Config.WeaponRepairPoints[k].RepairingData = {}
    TriggerClientEvent('weapons:client:SyncRepairShops', -1, Config.WeaponRepairPoints[k], k)
end)

CTRPFW.Functions.CreateUseableItem("pistol_suppressor", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "pistol_suppressor")
end)

CTRPFW.Functions.CreateUseableItem("pistol_extendedclip", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "pistol_extendedclip")
end)

CTRPFW.Functions.CreateUseableItem("smg_extendedclip", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "smg_extendedclip")
end)

CTRPFW.Functions.CreateUseableItem("smg_suppressor", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "smg_suppressor")
end)

CTRPFW.Functions.CreateUseableItem("smg_flashlight", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "smg_flashlight")
end)

CTRPFW.Functions.CreateUseableItem("smg_scope", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "smg_scope")
end)

CTRPFW.Functions.CreateUseableItem("rifle_suppressor", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "rifle_suppressor")
end)

CTRPFW.Functions.CreateUseableItem("rifle_extendedclip", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "rifle_extendedclip")
end)

CTRPFW.Functions.CreateUseableItem("rifle_drummag", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("CTRPFW:Notify", src, "You already have one "..AttachmentData.label:lower().." sit on your gun.", "error", 3500)
            end
        else
...
CTRPFW.Functions.CreateCallback('weapons:server:RemoveAttachment', function(source, cb, AttachmentData, ItemData)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
    local Inventory = Player.PlayerData.items
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, CTRPFW.Shared.Items[AttachmentComponent.item], "add")
                TriggerClientEvent("CTRPFW:Notify", src, "You have the "..AttachmentComponent.label.." removed from your weapon!", "error")

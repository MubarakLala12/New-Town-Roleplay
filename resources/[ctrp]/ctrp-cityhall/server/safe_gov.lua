...
Citizen.CreateThread(function()
    CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `moneysafesgov`", function(safes)
        if safes[1] ~= nil then
            for _, d in pairs(safes) do
                for safe, s in pairs(Config.Safes) do
                    if d.safe == safe then
                        ...
                        TriggerClientEvent('ctrp-moneysafesgov:client:UpdateSafe', -1, Config.Safes[safe], safe)
                    end
                end
            end
        end
...
CTRPFW.Commands.Add("govdeposit", "Put money in the safe", {}, false, function(source, args)
    local src = source
    ...
    if Player.PlayerData.job.name == "government" and Player.PlayerData.job.gradelabel == "Governor" then
        TriggerClientEvent('ctrp-moneysafesgov:client:DepositMoney', source, amount)
    else
        TriggerClientEvent('chatMessage', src, "SYSTEM", "error", "You are not authorised!")
    end
end)

CTRPFW.Commands.Add("govwithdraw", "Take money out of safe", {}, false, function(source, args)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(source)
    local amount = tonumber(args[1]) or 0

    if Player.PlayerData.job.name == "government" and Player.PlayerData.job.gradelabel == "Governor" then
        TriggerClientEvent('ctrp-moneysafesgov:client:WithdrawMoney', source, amount)
    else
        TriggerClientEvent('chatMessage', src, "SYSTEM", "error", "You are not authorised!")
    end
end)
...
RegisterServerEvent('ctrp-moneysafesgov:server:DepositMoney')
AddEventHandler('ctrp-moneysafesgov:server:DepositMoney', function(safe, amount, sender)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
    ...
    CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `moneysafesgov` WHERE `safe` = '"..safe.."'", function(result)
        if result[1] ~= nil then
            Config.Safes[safe].money = (Config.Safes[safe].money + amount)
            CTRPFW.Functions.ExecuteSql(false, "UPDATE `moneysafesgov` SET money = '"..Config.Safes[safe].money.."', transactions = '"..json.encode(Config.Safes[safe].transactions).."' WHERE `safe` = '"..safe.."'")
        else
            Config.Safes[safe].money = amount
            CTRPFW.Functions.ExecuteSql(false, "INSERT INTO `moneysafesgov` (`safe`, `money`, `transactions`) VALUES ('"..safe.."', '"..Config.Safes[safe].money.."', '"..json.encode(Config.Safes[safe].transactions).."')")
        end
        TriggerClientEvent('ctrp-moneysafesgov:client:UpdateSafe', -1, Config.Safes[safe], safe)
...
RegisterServerEvent('ctrp-moneysafesgov:server:WithdrawMoney')
AddEventHandler('ctrp-moneysafesgov:server:WithdrawMoney', function(safe, amount)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)

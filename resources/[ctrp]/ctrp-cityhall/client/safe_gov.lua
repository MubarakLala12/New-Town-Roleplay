        ...
        if ClosestSafe ~= nil then
            if PlayerData.job.name == ClosestSafe then
                if not IsAuthorized then
                    ...
                    if PlayerJob.name == 'government' and PlayerJob.gradelabel == "Governor" then
                        if distance < 20 then
                            inRange = true
                            if distance < Config.MinimumSafeDistance then
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '~g~$'..data.money)
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z - 0.1, '~b~/govdeposit~w~ - Put money in the safe')
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z - 0.2, '~b~/govwithdraw~w~ - Take money out of the safe')
                            end
                        end
                    end
                else
...
RegisterNetEvent('ctrp-moneysafesgov:client:DepositMoney')
AddEventHandler('ctrp-moneysafesgov:client:DepositMoney', function(amount)
    if ClosestSafe ~= nil then
        if not IsAuthorized then
            local ped = PlayerPedId()
            local pos = GetEntityCoords(ped)
            local data = Config.Safes[ClosestSafe]
                ...
                TriggerServerEvent('ctrp-moneysafesgov:server:DepositMoney', ClosestSafe, amount)
        end
    end
end)

RegisterNetEvent('ctrp-moneysafesgov:client:WithdrawMoney')
AddEventHandler('ctrp-moneysafesgov:client:WithdrawMoney', function(amount)
    if ClosestSafe ~= nil then
        if not IsAuthorized then
            local ped = PlayerPedId()
            local pos = GetEntityCoords(ped)
            ...
            if distance < Config.MinimumSafeDistance then
                TriggerServerEvent('ctrp-moneysafesgov:server:WithdrawMoney', ClosestSafe, amount)
            end
        end
    end
end)
...
RegisterNetEvent('ctrp-moneysafesgov:client:UpdateSafe')
AddEventHandler('ctrp-moneysafesgov:client:UpdateSafe', function(SafeData, k)

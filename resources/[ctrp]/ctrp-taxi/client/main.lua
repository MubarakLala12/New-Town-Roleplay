...
RegisterNetEvent('ctrp-taxi:client:DoTaxiNpc')
AddEventHandler('ctrp-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
        if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                CTRPFW.Functions.Notify('The NPC is indicated on your navigation!', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    CTRPFW.Functions.Notify('Bring the NPC to the specified location.')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                CTRPFW.Functions.Notify('You are already doing an NPC mission..')
            end
        else
            CTRPFW.Functions.Notify('No NPCs are available..')
        end
    else
        CTRPFW.Functions.Notify('You are not in a Taxi :(')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        ...
                        TriggerServerEvent('ctrp-taxi:server:NpcPay', meterData.currentFare)
                        CTRPFW.Functions.Notify('Person in top condition', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent('ctrp-taxi:client:toggleMeter')
AddEventHandler('ctrp-taxi:client:toggleMeter', function()
    local ped = GetPlayerPed(-1)
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            CTRPFW.Functions.Notify('This vehicle has no Taxi Meter..', 'error')
        end
    else
...
RegisterNetEvent('ctrp-taxi:client:enableMeter')
AddEventHandler('ctrp-taxi:client:enableMeter', function()
    local ped = GetPlayerPed(-1)
    ...
    if meterIsOpen then
        ...
        CTRPFW.Functions.Notify('The Taxi Meter is not active..', 'error')
    end
end)
...
RegisterNetEvent('ctrp-taxi:client:toggleMuis')
AddEventHandler('ctrp-taxi:client:toggleMuis', function()
    Citizen.Wait(400)
    if meterIsOpen then
        ...
        CTRPFW.Functions.Notify('No Taxi Meter in sight..', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    CTRPFW.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)

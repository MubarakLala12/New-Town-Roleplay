                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('ctrp-carwash:server:washCar')
                                    washVirus()
                                end
                            else
...
RegisterNetEvent('ctrp-carwash:client:washCar')
AddEventHandler('ctrp-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    CTRPFW.Functions.Progressbar("search_cabin", "Vehicle is being washed..", math.random(4000, 8000), false, true, {
        ...
        CTRPFW.Functions.Notify("Washing canceled..", "error")
        washingVehicle = false
    end)
end)

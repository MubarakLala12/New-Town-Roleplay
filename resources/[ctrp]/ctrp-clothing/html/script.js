...
CTRPClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('http://ctrp-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('http://ctrp-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('http://ctrp-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
                    $("#current-model").html("<p>"+model+"</p>")
                });
                canChange = true;
                CTRPClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    CTRPClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('http://ctrp-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('http://ctrp-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('http://ctrp-clothing/rotateRight');
            break;
        case 65: // A
            $.post('http://ctrp-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
CTRPClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                CTRPClothing.Open(event.data);
                break;
            case "close":
                CTRPClothing.Close();
                break;
            case "updateMax":
                CTRPClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                CTRPClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                CTRPClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                CTRPClothing.ResetValues();
                break;
            case "HideFaceAndModel":
                CTRPClothing.HideFaceAndModel(event.data.toggle);
            break;
        }
    })
});

CTRPClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    CTRPClothing.Close();
    $.post('http://ctrp-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    CTRPClothing.Close();
    $.post('http://ctrp-clothing/resetOutfit');
});

CTRPClothing.SetCurrentValues = function(clothingValues) {
...
CTRPClothing.Open = function(data) {
    ...
    CTRPClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    CTRPClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('http://ctrp-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('http://ctrp-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
CTRPClothing.Close = function() {
    $.post('http://ctrp-clothing/close');
...
CTRPClothing.SetMaxValues = function(maxValues) {
...
CTRPClothing.ResetValues = function() {
...
CTRPClothing.HideFaceAndModel = function(toggle) {
    if(toggle){
        $("div[data-type='face']").hide();
        $("div[data-type='model']").hide();
        $("div[data-type='eye_color']").hide();
        $("div[data-type='moles']").hide();
        $("div[data-type='nose_0']").hide();
        $("div[data-type='nose_1']").hide();
        $("div[data-type='nose_2']").hide();
        $("div[data-type='nose_3']").hide();
        $("div[data-type='nose_4']").hide();
        $("div[data-type='nose_5']").hide();
        $("div[data-type='eyebrown_high']").hide();
        $("div[data-type='eyebrown_forward']").hide();
        $("div[data-type='cheek_1']").hide();
        $("div[data-type='cheek_2']").hide();
        $("div[data-type='cheek_3']").hide();
        $("div[data-type='eye_opening']").hide();
        $("div[data-type='lips_thickness']").hide();
        $("div[data-type='jaw_bone_width']").hide();
        $("div[data-type='jaw_bone_back_lenght']").hide();
        $("div[data-type='chimp_bone_lowering']").hide();
        $("div[data-type='chimp_bone_lenght']").hide();
        $("div[data-type='chimp_bone_width']").hide();
        $("div[data-type='chimp_hole']").hide();
        $("div[data-type='neck_thikness']").hide();
    }
    else{
        $("div[data-type='face']").show();
        $("div[data-type='model']").show();
        $("div[data-type='eye_color']").show();
        $("div[data-type='moles']").show();
        $("div[data-type='nose_0']").show();
        $("div[data-type='nose_1']").show();
        $("div[data-type='nose_2']").show();
        $("div[data-type='nose_3']").show();
        $("div[data-type='nose_4']").show();
        $("div[data-type='nose_5']").show();
        $("div[data-type='eyebrown_high']").show();
        $("div[data-type='eyebrown_forward']").show();
        $("div[data-type='cheek_1']").show();
        $("div[data-type='cheek_2']").show();
        $("div[data-type='cheek_3']").show();
        $("div[data-type='eye_opening']").show();
        $("div[data-type='lips_thickness']").show();
        $("div[data-type='jaw_bone_width']").show();
        $("div[data-type='jaw_bone_back_lenght']").show();
        $("div[data-type='chimp_bone_lowering']").show();
        $("div[data-type='chimp_bone_lenght']").show();
        $("div[data-type='chimp_bone_width']").show();
        $("div[data-type='chimp_hole']").show();
        $("div[data-type='neck_thikness']").show();
    }
}

CTRPClothing.HideFaceAndModel2 = function(toggle) {
    if(toggle){
        $("div[data-type='eye_color']").hide();
        $("div[data-type='moles']").hide();
        $("div[data-type='nose_0']").hide();
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('http://ctrp-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('http://ctrp-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});

...
AddEventHandler('CTRPFW:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ctrp-clothes:loadPlayerSkin")
...
--[[RegisterNetEvent('CTRPFW:Client:Refreshskin')
AddEventHandler('CTRPFW:Client:Refreshskin', function()
    TriggerServerEvent("ctrp-clothes:loadPlayerSkin")
end)]]

RegisterNetEvent('CTRPFW:Client:Refreshskin')
AddEventHandler('CTRPFW:Client:Refreshskin', function()
    --TriggerServerEvent('ctrp-clothing:saveSkin')
    Citizen.Wait(100)
    TriggerServerEvent("ctrp-clothes:loadPlayerSkins")
end)
                        ...
                        if dist < 1 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    gender = "male"
                                    if CTRPFW.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('ctrp-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('ctrp-clothing:client:openMenu')
AddEventHandler('ctrp-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
function GetMaxValues()
    maxModelValues = {
        ...
        ["hair2"]        = {type = "hair", item = 0, texture = 0},
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('ctrp-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('ctrp-clothing:client:reloadOutfits')
AddEventHandler('ctrp-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
function resetClothing(data)
    ...
    SetPedHairColor(ped, data["hair"].texture, data["hair2"].texture)
    SetPedHairColor(ped, data["hair"].texture, data["hair2"].texture)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('ctrp-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    TriggerEvent('chatMessage', "SYSTEM", "warning", "You have "..data.outfitName.." deleted!")
end)
...
function ChangeVariation(data)
    ...
    elseif clothingCategory == "hair" then
        ...
        elseif type == "texture" then
            SetPedHairColor(ped, item,GetPedHairHighlightColor(ped))
            skinData["hair"].texture = item
        end
    elseif clothingCategory == "hair2" then
        if type == "item" then
        elseif type == "texture" then
            SetPedHairColor(ped, GetPedHairColor(ped), item)
            skinData["hair2"].texture = item
        end
...
function SaveSkin()
	...
	TriggerServerEvent("ctrp-clothing:saveSkin", model, clothing, cut)
end

RegisterNetEvent('ctrp-clothes:client:CreateFirstCharacter')
AddEventHandler('ctrp-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("ctrp-clothes:loadSkin")
AddEventHandler("ctrp-clothes:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('ctrp-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

--[[RegisterNetEvent("ctrp-clothes:loadSkins")
AddEventHandler("ctrp-clothes:loadSkins", function(new, model, data)
    model = model ~= nil and tonumber(model) or false
    ...
    Citizen.CreateThread(function()
        ...
        Citizen.Wait(10)
        TriggerServerEvent("ctrp-clothing:saveSkin" , PlayerPedId())
        Citizen.Wait(100)
...
RegisterNetEvent('ctrp-clothing:client:loadPlayerClothing')
AddEventHandler('ctrp-clothing:client:loadPlayerClothing', function(data, ped)
    ...
    -- SetPedHairColor(ped, data["hair"].texture, data["hair2"].texture)
    -- SetPedHairColor(ped, data["hair"].texture, data["hair2"].texture)
    if data["hair2"].texture == nil then
...
RegisterNetEvent('ctrp-clothing:client:loadOutfit')
AddEventHandler('ctrp-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
...
RegisterNetEvent("ctrp-clothing:client:adjustfacewear")
AddEventHandler("ctrp-clothing:client:adjustfacewear",function(type)
    if CTRPFW.Functions.GetPlayerData().metadata["ishandcuffed"] then return end

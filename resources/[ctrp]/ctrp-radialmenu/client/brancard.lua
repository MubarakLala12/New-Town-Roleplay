...
AddEventHandler('hospital:client:TakeBrancard', function()
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            CTRPFW.Functions.Notify("Something went wrong my friend..", 'error')
        end
    else
        CTRPFW.Functions.Notify("Your not near a ambulance..", 'error')
    end
end)
    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
            if DoesEntityExist(BrancardObject) then
                ...
                TriggerServerEvent('ctrp-radialmenu:server:RemoveBrancard', PlayerPos, BrancardObject)
                IsAttached = false
                BrancardObject = nil
                IsLayingOnBed = false
            end
        else
            CTRPFW.Functions.Notify('Your to far away!', 'error')
        end
    end
end)
...
RegisterNetEvent('ctrp-radialmenu:client:RemoveBrancardFromArea')
AddEventHandler('ctrp-radialmenu:client:RemoveBrancardFromArea', function(PlayerPos, BObject)
    local Ped = GetPlayerPed(-1)
...
function LayOnBrancard()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('ctrp-radialmenu:Brancard:BusyCheck', GetPlayerServerId(player), "lay")
        else
            LoadAnim(inBedDicts)
            if Object ~= nil or Object ~= 0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
RegisterNetEvent('ctrp-radialmenu:Brancard:client:BusyCheck')
AddEventHandler('ctrp-radialmenu:Brancard:client:BusyCheck', function(OtherId, type)
    local ped = GetPlayerPed(-1)
    if type == "lay" then
        ...
        if IsEntityPlayingAnim(ped, "anim@gangops@morgue@table@", "ko_front", 3) then
            TriggerServerEvent('ctrp-radialmenu:server:BusyResult', true, OtherId, type)
        else
            TriggerServerEvent('ctrp-radialmenu:server:BusyResult', false, OtherId, type)
        end
    else
...
RegisterNetEvent('ctrp-radialmenu:client:Result')
AddEventHandler('ctrp-radialmenu:client:Result', function(IsBusy, type)
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            CTRPFW.Functions.Notify("This stretcher is already in use!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
            ...
            CTRPFW.Functions.Notify("This brancard is already in use!", "error")
            IsAttached = false
        end
    end
end)
...
function AttachToBrancard()
    ...
    if BrancardObject ~= nil then
        if ClosestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('ctrp-radialmenu:Brancard:BusyCheck', GetPlayerServerId(ClosestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(BrancardObject)
                LoadAnim("anim@heists@box_carry@")
                TaskPlayAnim(PlayerPed, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)

...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    CTRPFW.Functions.ExecuteSql(false, "INSERT INTO `apartments` (`name`, `type`, `label`, `citizenid`) VALUES ('"..apartmentId.."', '"..type.."', '"..label.."', '"..Player.PlayerData.citizenid.."')")
    TriggerClientEvent('CTRPFW:Notify', src, "You got a apartment ("..label..")")
...
AddEventHandler('apartments:server:UpdateApartment', function(type)
    ...
    CTRPFW.Functions.ExecuteSql(false, "UPDATE `apartments` SET type='"..type.."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")

    TriggerClientEvent('CTRPFW:Notify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
function CreateApartmentId(type)
	...
	while not UniqueFound do
		...
		CTRPFW.Functions.ExecuteSql(true, "SELECT COUNT(*) as count FROM `apartments` WHERE `name` = '"..tostring(type .. AparmentId).."'", function(result)
...
function GetApartmentInfo(apartmentId)
    local retval = nil
    CTRPFW.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `name` = '"..apartmentId.."' ", function(result)
...
function GetOwnedApartment(citizenid)
    CTRPFW.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `citizenid` = '"..citizenid.."' ", function(result)
        if result[1] ~= nil then 
            return result[1]
        end
        return nil
...
CTRPFW.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
CTRPFW.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
CTRPFW.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
CTRPFW.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
        CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..cid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
        ...
        local src = source
        local Player = CTRPFW.Functions.GetPlayer(src)
        CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
...
CTRPFW.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
    if Player ~= nil then
        CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
...
CTRPFW.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = CTRPFW.Functions.GetPlayer(src)
...
RegisterServerEvent('ctrp-apartments:server:SetInsideMeta')
AddEventHandler('ctrp-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)

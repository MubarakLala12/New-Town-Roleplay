...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = CTRPFW.Shared.Items["security_card_01"]["name"], image = CTRPFW.Shared.Items["security_card_01"]["image"]},
    }
                            ...
                            if not Config.BigBanks["paleto"]["lockers"][k]["isOpened"] then
                                if lockerDist < 5 then
                                    ...
                                    if lockerDist < 0.5 then
                                        DrawText3Ds(Config.BigBanks["paleto"]["lockers"][k].x, Config.BigBanks["paleto"]["lockers"][k].y, Config.BigBanks["paleto"]["lockers"][k].z + 0.3, '[E] Break the safe')
...
RegisterNetEvent('ctrp-bankrobbery:UseBankcardA')
AddEventHandler('ctrp-bankrobbery:UseBankcardA', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dist = GetDistanceBetweenCoords(pos, Config.BigBanks["paleto"]["coords"]["x"], Config.BigBanks["paleto"]["coords"]["y"],Config.BigBanks["paleto"]["coords"]["z"])
        ...
        CTRPFW.Functions.TriggerCallback('ctrp-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        CTRPFW.Functions.Progressbar("security_pass", "Using pass..", math.random(5000, 10000), false, true, {
                            ...
                            TriggerServerEvent('ctrp-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("CTRPFW:Server:RemoveItem", "security_card_01", 1)
                            TriggerServerEvent('ctrp-doorlock:server:updateState', 91, false)
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("ctrp-bankrobbery:server:callCops", "paleto", 0, streetLabel, pos)
                                    copsCalled = true
                                end
                            end
                        end, function() -- Cancel
                ...
                CTRPFW.Functions.Notify("The security lock is active, the door cannot be opened at the moment..", "error", 5500)
            end
        end)
    end 
end)
...
function OpenPaletoDoor()
    TriggerServerEvent('ctrp-doorlock:server:updateState', 85, false)

...
AddEventHandler("CTRPFW:Client:OnPlayerLoaded", function()
    TriggerServerEvent("ctrp-scrapyard:server:LoadVehicleList")
end)
							...
							if IsControlJustReleased(0, Keys["E"]) then
								if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										CTRPFW.Functions.Notify("This vehicle cannot be scrapped..", "error")
									end
								else
									CTRPFW.Functions.Notify("You are not the driver..", "error")
								end
							end
						end
					end
...
RegisterNetEvent('ctrp-scrapyard:client:setNewVehicles')
AddEventHandler('ctrp-scrapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = CTRPFW.Shared.Vehicles[v]
		...
		SetTimeout(math.random(15000, 20000), function()
			...
			TriggerServerEvent('ctrp-phone:server:sendNewMail', {
				sender = "Paulie B.",
				subject = "Vehicles List",
				message = "You can scrap a limited # of vehicles<br />You can keep everything just dont bother me!<br /><br /><strong>Vehicle List:</strong><br />".. vehicleList,
				button = {}
			})
		end)
	else
		CTRPFW.Functions.Notify("You may can't scrap vehicles right now..", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	CTRPFW.Functions.Progressbar("scrap_vehicle", "Chopping vehicle..", scrapTime, false, true, {
		...
		TriggerServerEvent("ctrp-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))

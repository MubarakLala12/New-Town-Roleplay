...
Citizen.CreateThread(function()
    Wait(1000)
    if CTRPFW.Functions.GetPlayerData().job ~= nil and next(CTRPFW.Functions.GetPlayerData().job) then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if math.random(100) <= 50 then
                                        TriggerServerEvent('virus-hudcomponent:Server:GainStress', math.random(1, 3))
                                    end
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        CTRPFW.Functions.TriggerCallback('ctrp-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    if not copsCalled then
                                        ...
                                        TriggerServerEvent("ctrp-storerobbery:server:callCops", "safe", currentSafe, streetLabel, pos)
                                        copsCalled = true
                                    end
                                else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if not copsCalled then
                        ...
                        print('1')
                        TriggerServerEvent("ctrp-storerobbery:server:callCops", "cashier", currentRegister, streetLabel, pos)
                        copsCalled = true
                    end
                else
                    CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(result)
                        if result then
                            ...
                            CTRPFW.Functions.Notify("It looks like you are missing a screwdriver..", "error")
                        end
                    end, "screwdriverset")
                end
...
function setupRegister()
    CTRPFW.Functions.TriggerCallback('ctrp-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    CTRPFW.Functions.TriggerCallback('ctrp-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        lockpick(false)
        TriggerServerEvent('ctrp-storerobbery:server:setRegisterStatus', currentRegister)
        local lockpickTime = 30000
        LockpickDoorAnim(lockpickTime)
        CTRPFW.Functions.Progressbar("search_register", "Searching..", lockpickTime, false, true, {
            ...
            TriggerServerEvent('ctrp-storerobbery:server:takeMoney', currentRegister, true)            
            currentRegister = 0
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(GetPlayerPed(-1))
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent("ctrp-storerobbery:server:SafeReward", currentSafe)
                    TriggerServerEvent("ctrp-storerobbery:server:setSafeStatus", currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
RegisterNUICallback('fail', function()
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            ...
            TriggerEvent('inventory:client:ItemBox', CTRPFW.Shared.Items["advancedlockpick"], "remove")
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent("CTRPFW:Server:RemoveItem", "lockpick", 1)
            TriggerEvent('inventory:client:ItemBox', CTRPFW.Shared.Items["lockpick"], "remove")
        end
    end
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        CTRPFW.Functions.Notify("You tore the lock pick..")
    end
    lockpick(false)
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    CTRPFW.Functions.TriggerCallback('ctrp-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('ctrp-storerobbery:client:setRegisterStatus')
AddEventHandler('ctrp-storerobbery:client:setRegisterStatus', function(register, bool)
    Config.Registers[register].robbed = bool
end)

RegisterNetEvent('ctrp-storerobbery:client:setSafeStatus')
AddEventHandler('ctrp-storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('ctrp-storerobbery:client:robberyCall')
AddEventHandler('ctrp-storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == 'police' and onDuty then
        ...
        TriggerEvent('ctrp-policealerts:client:AddPoliceAlert', {
            timeOut = 5000,
            alertTitle = "10-90C | Shop robbery",
            coords = {
                x = coords.x,
                y = coords.y,
                z = coords.z,

...
positions = {
    ...
    {{935.98, 47.14, 81.1, 0}, {1089.72, 205.78, -49.0, 358},{255,0,0}, "Diamond Casino"}, 
}
    ...
    while true do
        ...
        for _,location in ipairs(positions) do
			...
			if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 2) then 
				DrawText3Ds(loc1.x, loc1.y, loc1.z + 0.5, 'Press [~g~E~s~] to Enter Casino')
			...
			elseif CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc2.x, loc2.y, loc2.z, 2) then
				DrawText3Ds(loc2.x, loc2.y, loc2.z + 0.5, 'Press [~g~E~s~] to go outside of Casino')
                if IsControlJustReleased(1, key_to_teleport) then
					DoScreenFadeOut(500)
    				Wait(500)
			...
			elseif CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, 1085.26, 214.16, -49.2, 2) then
				DrawText3Ds(1085.26, 214.16, -49.2 + 0.5, 'Press [~g~E~s~]- Take the elevator to top')
                if IsControlJustReleased(1, key_to_teleport) then
					DoScreenFadeOut(500)
    				Wait(500)
                    SetEntityCoords(player, 965.19, 58.52, 112.55)
                    SetEntityHeading(player, 56.32)
					DoScreenFadeIn(500)
					enterCasino(false)
		            Citizen.Wait(1)
			        enterCasino(false)
			...
			elseif CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, 965.19, 58.52, 112.55, 2) then
				DrawText3Ds(965.19, 58.52, 112.55 + 0.5, 'Press [~g~E~s~]- Take the elevator down')
                if IsControlJustReleased(1, key_to_teleport) then
					DoScreenFadeOut(500)
    				Wait(500)
                    SetEntityCoords(player, 1085.26, 214.16, -49.2)
                    SetEntityHeading(player, 323.3)
					DoScreenFadeIn(500)
					enterCasino(false)
		            Citizen.Wait(1)
			        enterCasino(true)
...
function drawCarForWins()
	...
	spinningCar = CreateVehicle(carOnShow, 1100.0, 220.0, -51.0 + 0.10, 0.0, 0, 0)
	Wait(0)
	SetVehicleDirtLevel(spinningCar, 0.0)
	SetVehicleOnGroundProperly(spinningCar)
	SetVehicleUndriveable(spinningCar, true)
	WashDecalsFromVehicle(spinningCar, 1.0)
	SetVehicleNumberPlateText(spinningCar, 'CASINO')
	SetVehicleColours(spinningCar, 12)
	Wait(0)

                ...
                if seat == ped then
                    if not CheckDone then
                        if vehiclemeters == -1 then
                            CheckDone = true
                            CTRPFW.Functions.TriggerCallback('ctrp-vehicletuning:server:IsVehicleOwned', function(IsOwned)
                                if IsOwned then
                ...
                if vehiclemeters ~= -1 then
                    if seat == ped then
                        if previousvehiclepos ~= nil then
                            ...
                            if DamageKey ~= nil then
                                ...
                                if chance == odd then
                                    for k, v in pairs(Config.Damages) do
                                        ...
                                        TriggerServerEvent('ctrp-vehicletuning:server:SetPartLevel', plate, k, newDamage)
                                    end
                                end
                            end
                            ...
                            TriggerServerEvent('ctrp-vehicletuning:server:UpdateDrivingDistance', DrivingDistance[plate], plate)
                        end
                    else
                        if invehicle then
                            if DrivingDistance[plate] ~= nil then
...
RegisterNetEvent('ctrp-vehicletuning:client:UpdateDrivingDistance')
AddEventHandler('ctrp-vehicletuning:client:UpdateDrivingDistance', function(amount, plate)

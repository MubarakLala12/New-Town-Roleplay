...
AddEventHandler('CTRPFW:Client:OnPlayerLoaded', function()
    ...
    isLoggedIn = true
    CTRPFW.Functions.TriggerCallback('ctrp-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    CTRPFW.Functions.TriggerCallback('ctrp-vehicletuning:server:GetDrivingDistances', function(retval)
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Los Santos Mechanic")
    ...
    while true do
        ...
        if isLoggedIn then
            if PlayerJob.name == "mechanic" then
                ...
                if onDuty then
                    inRange = true
                    if (GetDistanceBetweenCoords(pos, Config.Locations["pstash"].x, Config.Locations["pstash"].y, Config.Locations["pstash"].z, true) < 1.5) then
                        DrawText3Ds(Config.Locations["pstash"].x, Config.Locations["pstash"].y, Config.Locations["pstash"].z, "[E] Personal Stash")
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("inventory:server:OpenInventory", "stash", "personalmecstash_"..CTRPFW.Functions.GetPlayerData().citizenid)
                            TriggerEvent("inventory:client:SetCurrentStash", "personalmecstash_"..CTRPFW.Functions.GetPlayerData().citizenid)
                        end
                    end
                end
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, Config.Keys["E"]) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('ctrp-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            CTRPFW.Functions.Notify("You cant put a bike on the plate!", "error")
                                        end
                                    end
                                end
                            end
                            ...
                            if PlateDistance < 2 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Menu of options")
                                if IsControlJustPressed(0, Keys["E"]) then
                                    OpenMenu()
                                    Menu.hidden = not Menu.hidden
                                end
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..CTRPFW.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close menu", "CloseMenu", nil)
end
...
function RepairPart(part)
    ...
    CTRPFW.Functions.TriggerCallback('ctrp-inventory:server:GetStashItems', function(StashItems)
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    CTRPFW.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." Repairing", math.random(5000, 10000), false, true, {
                        ...
                        TriggerEvent('ctrp-vehicletuning:client:RepaireeePart', part)
                        TriggerServerEvent('ctrp-inventory:server:SaveStashItems', "mechanicstash", StashItems)
                        SetTimeout(250, function()
                            PartsMenu()
                        end)
                    end, function()
                        StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
                        CTRPFW.Functions.Notify("Repair canceled..", "error")
                    end)
                    break
                else
                    CTRPFW.Functions.Notify('There are not enough materials in the safe..', 'error')
                end
                break
            else
                CTRPFW.Functions.Notify('There are not enough materials in the safe..', 'error')
...
RegisterNetEvent('ctrp-vehicletuning:client:RepaireeePart')
AddEventHandler('ctrp-vehicletuning:client:RepaireeePart', function(part)
    ...
    CTRPFW.Functions.Notify("The "..Config.ValuesLabels[part].."is repaired!")
end)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('ctrp-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('ctrp-vehicletuning:client:SetAttachedVehicle')
AddEventHandler('ctrp-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then
                        ...
                        CTRPFW.Functions.Notify("No status know..", "error")
                    end
                else
                    CTRPFW.Functions.Notify("Not a valid vehicle..", "error")
                end
            else
                CTRPFW.Functions.Notify("You are not close enough to the vehicle..", "error")
            end
        else
            CTRPFW.Functions.Notify("You must be in the vehicle first..", "error")
        end
    else
        CTRPFW.Functions.Notify("You must be outside the vehicle..", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
            ...
            CTRPFW.Functions.Notify("You are not a driver or on a bicycle..", "error")
        end
    else
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            CTRPFW.Functions.Progressbar("repair_advanced", "Repair vehicle..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerServerEvent("CTRPFW:Server:RemoveItem", Config.RepairCost[part], needAmount)
                                    TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items[Config.RepairCost[part]], "remove")
                                elseif part ~= "engine" then
                                    TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                    TriggerServerEvent("CTRPFW:Server:RemoveItem", Config.RepairCost[part], level)
                                    TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items[Config.RepairCost[part]], "remove")
                                end
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(GetPlayerPed(-1))
                                CTRPFW.Functions.Notify("Process canceled..", "error")
                            end)
                        else
                            CTRPFW.Functions.Notify("Not a valid part..", "error")
                        end
                    else
                        CTRPFW.Functions.Notify("Not a valid vehicle..", "error")
                    end
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['ctrp-companies']:IsEmployee(v)
    end
    return retval
end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Vehicle Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..CTRPFW.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..CTRPFW.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..CTRPFW.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..CTRPFW.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..CTRPFW.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..CTRPFW.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..CTRPFW.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end

        ...
        if ClosestSafe ~= nil then
            if PlayerData.job.name == ClosestSafe then
                if not IsAuthorized then
                    ...
                    if PlayerJob.name == 'mechanic' and PlayerJob.gradelabel == "Boss" then
                        if distance < 20 then
                            inRange = true
                            if distance < Config.MinimumSafeDistance then
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '~g~$'..data.money)
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z - 0.1, '~b~/mecdeposit~w~ - Put money in the safe')
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z - 0.2, '~b~/mecwithdraw~w~ - Take money out of the safe')
                            end
                        end
                    end
                else
...
RegisterNetEvent('ctrp-moneysafesmec:client:DepositMoney')
AddEventHandler('ctrp-moneysafesmec:client:DepositMoney', function(amount)
    if ClosestSafe ~= nil then
        if not IsAuthorized then
            local ped = GetPlayerPed(-1)
            local pos = GetEntityCoords(ped)
            local data = Config.Safes[ClosestSafe]
                ...
                TriggerServerEvent('ctrp-moneysafesmec:server:DepositMoney', ClosestSafe, amount)
        end
    end
end)

RegisterNetEvent('ctrp-moneysafesmec:client:WithdrawMoney')
AddEventHandler('ctrp-moneysafesmec:client:WithdrawMoney', function(amount)
    if ClosestSafe ~= nil then
        if not IsAuthorized then
            local ped = GetPlayerPed(-1)
            local pos = GetEntityCoords(ped)
            ...
            if distance < Config.MinimumSafeDistance then
                TriggerServerEvent('ctrp-moneysafesmec:server:WithdrawMoney', ClosestSafe, amount)
            end
        end
    end
end)
...
RegisterNetEvent('ctrp-moneysafesmec:client:UpdateSafe')
AddEventHandler('ctrp-moneysafesmec:client:UpdateSafe', function(SafeData, k)

...
function UpdateBlip()
    Citizen.CreateThread(function()
        ...
        SetBlipSprite(HotdogBlip, 436)
        SetBlipDisplay(HotdogBlip, 4)
        SetBlipScale(HotdogBlip, 0.7)
        SetBlipAsShortRange(HotdogBlip, true)
        SetBlipColour(HotdogBlip, 5)
...
function StartWorking()
    CTRPFW.Functions.TriggerCallback('ctrp-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            local PlayerPed = GetPlayerPed(-1)
            ...
            UpdateUI()
            CheckLoop()
            CTRPFW.Functions.Notify('You have paid a $ 1000 deposit!..', 'success')
        else
            CTRPFW.Functions.Notify('You have no money for the deposit..', 'error')
        end
    end)
end
...
RegisterNetEvent('ctrp-hotdogjob:client:UpdateReputation')
AddEventHandler('ctrp-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('ctrp-hotdogjob:client:ToggleSell')
AddEventHandler('ctrp-hotdogjob:client:ToggleSell', function(data)
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            CTRPFW.Functions.Notify('You are too far from your Hot Dog Stand..', 'error')
        end
    else
        CTRPFW.Functions.Notify('You don\'t have a Hotdog stand', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 15.0 then
            ...
            CTRPFW.Functions.Notify('You are too far from your stall..', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                CTRPFW.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sale '..HotdogsForSale..'x for $'..(HotdogsForSale * SellingPrice)..',- / [8]   Reject')
                if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                    CTRPFW.Functions.Notify(HotdogsForSale..'x Hotdog(\'s) sold for $'..(HotdogsForSale * SellingPrice)..',-', 'success')
                    TriggerServerEvent('ctrp-hotdogjob:server:Sell', HotdogsForSale, SellingPrice)
                    SellingData.HasTarget = false
                    ...
                    TriggerServerEvent('ctrp-hotdogjob:removeHotDog')
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    CTRPFW.Functions.Notify('Customer refused!', 'error')
                    SellingData.HasTarget = false
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                CTRPFW.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, 'You have no stock .. / [8] Refuse customer')
...
function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('ctrp-keyminigame:show')
    TriggerEvent('ctrp-keyminigame:start', FinishMinigame)
end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('ctrp-hotdogjob:server:UpdateReputation', Quality)
        if Config.MyLevel == 1 then
            CTRPFW.Functions.Notify('You have made a '..Config.Stock[Quality].Label..' Hotdog!')
            Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
            TriggerServerEvent('ctrp-hotdogjob:getHotDog')
        else
            ...
            if Luck == LuckyNumber then
                CTRPFW.Functions.Notify('you have '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s made!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + LuckyAmount
                TriggerServerEvent('ctrp-hotdogjob:getHotDog')
            else
                CTRPFW.Functions.Notify('You have made '..Config.Stock[Quality].Label..' a Hotdog !')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
                TriggerServerEvent('ctrp-hotdogjob:getHotDog')
            end
        end
    else
        CTRPFW.Functions.Notify('You do not have ('..Config.Stock[Quality].Label..') stock more about it..')
    end
    PreparingFood = false
end
...
function StopWorking()
    if DoesEntityExist(StandObject) then
        CTRPFW.Functions.TriggerCallback('ctrp-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                CTRPFW.Functions.Notify('You have received your $ 1000 deposit back!', 'success')
            else
                CTRPFW.Functions.Notify('Something is wrong..', 'error')
            end
        end)
    else
        CTRPFW.Functions.Notify('Your hot dog stall was nowhere to be seen, you did NOT receive your deposit back!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
RegisterNetEvent('ctrp-hotdogjob:staff:DeletStand')
AddEventHandler('ctrp-hotdogjob:staff:DeletStand', function()
    local ped = GetPlayerPed(-1)
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)
            DeleteEntity(Object)
            CTRPFW.Functions.Notify('Hot dog mode removed!')
        end
    end
end)

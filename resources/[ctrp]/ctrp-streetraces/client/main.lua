                    ...
                    if Races[k] ~= nil then
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[k].startx, Races[k].starty, Races[k].startz, true) < 15.0 and not Races[k].started then
                            ...
                            if IsControlJustReleased(0, Keys["H"]) then
                                TriggerServerEvent("ctrp-streetraces:JoinRace", k)
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and InRace then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 250.0 and Races[RaceId].started then
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 15.0 then
                        TriggerServerEvent("ctrp-streetraces:RaceWon", RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('ctrp-streetraces:StartRace')
AddEventHandler('ctrp-streetraces:StartRace', function(race)
...
RegisterNetEvent('ctrp-streetraces:RaceDone')
AddEventHandler('ctrp-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        CTRPFW.Functions.Notify(" The race endded. "..winner.. "!")
    end
end)

RegisterNetEvent('ctrp-streetraces:StopRace')
AddEventHandler('ctrp-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('ctrp-streetraces:CreateRace')
AddEventHandler('ctrp-streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(GetPlayerPed(-1), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            TriggerServerEvent("ctrp-streetraces:NewRace", race)
            CTRPFW.Functions.Notify("Race made for $"..amount..",-!", "success")
        else
            CTRPFW.Functions.Notify("end position to close!", "error")
        end
    else
        CTRPFW.Functions.Notify("You have to place a marker!", "error")
    end
end)

RegisterNetEvent('ctrp-streetraces:SetRace')
AddEventHandler('ctrp-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('ctrp-streetraces:SetRaceId')
AddEventHandler('ctrp-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        CTRPFW.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    CTRPFW.Functions.Notify("GOOOOOOOOO!!!")
end

	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				CTRPFW.Functions.Notify("You have gone out of range!")
			end
		end
	end
end)
...
RegisterNetEvent("ctrp-houses:client:decorate")
AddEventHandler("ctrp-houses:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
			...
			CTRPFW.Functions.Notify("You must have the keys to the house!", "error")
		end
	else
...
RegisterNetEvent("ctrp-houses:server:sethousedecorations")
AddEventHandler("ctrp-houses:server:sethousedecorations", function(house, decorations)
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    ...
    TriggerServerEvent('ctrp-houses:server:buyobject' , objectData)
end)

RegisterNetEvent("ctrp-houses:client:donekardo")
AddEventHandler("ctrp-houses:client:donekardo", function()
    SetNuiFocus(false, false)
    cursorEnabled = not cursorEnabled
    SaveDecorations()
    SelectedObj = nil
...
RegisterNetEvent("ctrp-houses:client:donekardo2")
AddEventHandler("ctrp-houses:client:donekardo2", function()
    DeleteObject(SelectedObj)
    SelectedObj = nil
    SelObjId = 0
    peanut = false
...
function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('ctrp-anticheat:client:ToggleDecorate', true)
end
...
function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('ctrp-anticheat:client:ToggleDecorate', false)
end
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		CTRPFW.Functions.TriggerCallback('ctrp-houses:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("ctrp-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		CTRPFW.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end

        ...
        if isLoggedIn then
            ...
            for k, v in pairs(Config.DeliveryPoints) do
                ...
                if dist < 30 then
                    ...
                    if dist < 1 then
                        DrawText3Ds(Config.DeliveryPoints[k]["coords"]["x"], Config.DeliveryPoints[k]["coords"]["y"], Config.DeliveryPoints[k]["coords"]["z"] + 0.35, '~g~E~w~ - Return vehicle')
                        if IsControlJustPressed(0, Keys["E"]) then
                            ReturnVehicle()
                            Menu.hidden = not Menu.hidden
                            CurrentRentalPoint = k
...
RentVehicle = function(selectedVehicle)
    ...
    TriggerServerEvent('ctrp-vehiclerental:server:SetVehicleRented', vehiclePlate, true, Config.RentalVehicles[selectedVehicle])

    RentedVehicleData = selectedVehicle
end

RegisterNetEvent('ctrp-vehiclerental:server:SpawnRentedVehicle')
AddEventHandler('ctrp-vehiclerental:server:SpawnRentedVehicle', function(vehiclePlate, vehicleData)
    local ped = GetPlayerPed(-1)
    local coords = {
...
AcceptReturn = function()
    ...
    if noSpace(VehiclePlate) ~= noSpace(RentedVehiclePlate) then
        ...
        TriggerServerEvent('ctrp-vehiclerental:server:SetVehicleRented', RentedVehiclePlate, false, Config.RentalVehicles[RentedVehicleData])

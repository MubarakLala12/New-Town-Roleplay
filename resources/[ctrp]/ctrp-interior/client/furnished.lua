...
RegisterNetEvent('ctrp-interior:client:SetNewState')
AddEventHandler('ctrp-interior:client:SetNewState', function(bool)
	IsNew = bool
end)
...
--[[function CreateApartmentFurnished(spawn)
	...
	POIOffsets.shower = json.decode('{"z":1.2,"y":-1.9900,"x":-4.590000,"h":2.2633972168}')
	POIOffsets.handwash = json.decode('{"z":1.2,"y":-1.60,"x":-2.124350097,"h":2.2633972168}')
...
function CreateApartmentFurnished(spawn)
    ...
    local POIOffsets = {}
    POIOffsets.exit = json.decode('{"z":1.3,"y":5.0,"x":0.05,"h":2.2633972168}')
    POIOffsets.clothes = json.decode('{"z":0.9,"y":1.793774,"x":4.359713,"h":2.2633972168}')
    POIOffsets.stash = json.decode('{"z":1.0,"y":4.15,"x":-3.05,"h":2.2633972168}')
    POIOffsets.logout = json.decode('{"z":0.7,"y":0.8,"x":-3.05,"h":2.2633972168}')
	--[[POIOffsets.shower = json.decode('{"z":1.2,"y":-1.9900,"x":-4.590000,"h":2.2633972168}')
	POIOffsets.handwash = json.decode('{"z":1.2,"y":-1.60,"x":-2.124350097,"h":2.2633972168}')]]
    ...
    RequestModel(`motel_shell`)
    while not HasModelLoaded(`motel_shell`) do
        Citizen.Wait(3)
    end
    local house = CreateObject(`motel_shell`, spawn.x, spawn.y, spawn.z, false, false, false)
    FreezeEntityPosition(house, true)
    table.insert(objects, house)

    TeleportToInterior(spawn.x + 0.05, spawn.y - 4.5, spawn.z + 0.45, POIOffsets.exit.h)
    if IsNew then
        SetTimeout(750, function()
            TriggerEvent('ctrp-clothes:client:CreateFirstCharacter')
            IsNew = false
...
-- function CreateFuryMotel(spawn)
-- 	local objects = {}

--     local POIOffsets = {}
-- 	POIOffsets.exit = json.decode('{"z":1.5,"y":3.40618,"x":-0.25,"h":2.2633972168}')
-- 	POIOffsets.clothes = json.decode('{"z":1.0,"y":1.5,"x":-4.25,"h":2.2633972168}')
-- 	POIOffsets.stash = json.decode('{"z":1.1,"y":2.8,"x":3.3,"h":2.2633972168}')
-- 	POIOffsets.logout = json.decode('{"z":1.2,"y":2.5,"x":-3.1,"h":2.2633972168}')
--     DoScreenFadeOut(500)
--     while not IsScreenFadedOut() do
--         Citizen.Wait(10)
--     end
-- 	RequestModel(`fury_apt_shell`)
-- 	while not HasModelLoaded(`fury_apt_shell`) do
-- 	    Citizen.Wait(3)
-- 	end
-- 	local house = CreateObject(`fury_apt_shell`, spawn.x, spawn.y, spawn.z, false, false, false)

--     FreezeEntityPosition(house, true)
-- 	table.insert(objects, house)

-- 	local furystorage = CreateObject(GetHashKey("v_res_fh_sidebrddine"),spawn.x - 3.3 ,spawn.y - 2.9,spawn.z + 0.1,false,false,false)
-- 	local furyflower = CreateObject(GetHashKey("vw_prop_flowers_vase_02a"),spawn.x-6.473187,spawn.y+3.606355,spawn.z + 0,false,false,false)

-- 	FreezeEntityPosition(furystorage,true)
-- 	FreezeEntityPosition(furyflower,true)

-- 	table.insert(objects, furystorage)
-- 	table.insert(objects, furyflower)

-- 	SetEntityHeading(furystorage,GetEntityHeading(furystorage) + 180)
-- 	SetEntityHeading(furyflower,GetEntityHeading(furyflower)+90)

-- 	TeleportToInterior(spawn.x + 0.2585847, spawn.y - 3.215818, spawn.z + 1.5, POIOffsets.exit.h)
-- 	if IsNew then
-- 		SetTimeout(750, function()
-- 			TriggerEvent('ctrp-clothes:client:CreateFirstCharacter')
-- 			IsNew = false

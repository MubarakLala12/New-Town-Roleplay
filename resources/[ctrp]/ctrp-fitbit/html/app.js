...
ctrpFitbit.Open = function() {
    $(".container").fadeIn(150);
}

ctrpFitbit.Close = function() {
    $(".container").fadeOut(150);
    $.post('http://ctrp-fitbit/close')
}
...
$(document).on('click', '.save-food-settings', function(e){
    ...
    if (parseInt(foodValue.val()) <= 100) {
        $.post('http://ctrp-fitbit/setFoodWarning', JSON.stringify({
            value: foodValue.val()
        }));
    }
});
...
$(document).on('click', '.save-thirst-settings', function(e){
    ...
    if (parseInt(thirstValue.val()) <= 100) {
        $.post('http://ctrp-fitbit/setThirstWarning', JSON.stringify({

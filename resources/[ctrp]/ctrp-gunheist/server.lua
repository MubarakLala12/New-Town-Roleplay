...
local dropItems = {
	...
	{itemName = "tunerlaptop", label = "Tuner Chip", chance = 32},
	...
	{itemName = "weapon_marksmanpistol", label = "Marksman Pistol", chance = 8},
	{itemName = "weapon_revolver", label = "Heavy Revolver", chance = 16},
...
RegisterServerEvent('ctrp-gunheist:wonHacking')
AddEventHandler('ctrp-gunheist:wonHacking', function()
	local src = source
	local Player = CTRPFW.Functions.GetPlayer(src)
	local chipSlot = Player.Functions.GetItemByName('virusdrive')
...
RegisterServerEvent('ctrp-gunheist:server:useDecryptionKey')
AddEventHandler('ctrp-gunheist:server:useDecryptionKey', function(item)
	local src = source
	local Player = CTRPFW.Functions.GetPlayer(src)
	...
	if Player then
		...
		if keySlot.amount > 0 then
			...
			if keys[activeKeys[keysUsed + 1]].itemName == item then
				keysUsed = keysUsed + 1
				if keysUsed == 3 then
					TriggerClientEvent('ctrp-gunheist:hackingMinigame', src)
				else
					TriggerClientEvent('CTRPFW:Notify', src, "Correct key!")
				end
			else
				TriggerClientEvent('CTRPFW:Notify', src, "Wrong key!", "error")
			end
		end
	end
end)

RegisterServerEvent('ctrp-gunheist:createHeistLoop')
AddEventHandler('ctrp-gunheist:createHeistLoop', function()
	heistId = heistId + 1
	...
	if heistId == thisId then
		...
		if heistId == thisId then
			TriggerEvent('ctrp-gunheist:createHeistLoop')
		end
	end
end)

RegisterServerEvent('ctrp-gunheist:obtainDrops')
AddEventHandler('ctrp-gunheist:obtainDrops', function()
	TriggerClientEvent('ctrp-gunheist:obtainDrops', source, activeDrops)
end)
...
AddEventHandler('checkDrill', function(dropId) 
	...
	if drill ~= nil and drill.amount > 0 then
		...
		TriggerClientEvent('CTRPFW:Notify', src, 'you have no drill!', 'error')
	end
end)

RegisterServerEvent('ctrp-gunheist:pickUpDrop')
AddEventHandler('ctrp-gunheist:pickUpDrop', function(dropId)
	local src = source
	local Player = CTRPFW.Functions.GetPlayer(src)
	...
	if activeDrops[dropId] then
		...
		if Player then
			if drillSlot ~= nil and drillSlot.amount >= 1 then
				...
				TriggerClientEvent('CTRPFW:Notify', src, "Picked up " .. dropData.label)
				TriggerClientEvent('clearTasks', src)

				TriggerClientEvent('ctrp-gunheist:manipulateDrop', -1, dropId)
				TriggerEvent("ctrp-log:server:CreateLog", "gunhiest", "GUNHIEST", "green", "**"..GetPlayerName(src) .. "** has Picked up " .. dropData.label)
			else
				TriggerClientEvent("CTRPFW:Notify", src, "You need to have a drill on you...", "error")
			end
		end
	end
end)

RegisterServerEvent('ctrp-gunheist:hackingCompleted')
AddEventHandler('ctrp-gunheist:hackingCompleted', function()
	local src = source
	...
	if not weaponDropped then
		...
		if item then
			...
			TriggerClientEvent('ctrp-gunheist:manipulateDrop', -1, dropIds, activeDrops[dropIds])
			TriggerClientEvent('ctrp-gunheist:createBlipOnDrop', src, dropIds, item.label)
			TriggerEvent('ctrp-gunheist:createHeistLoop')
		end
	end
end)
...
CTRPFW.Functions.CreateUseableItem("virusdrive", function(source, item) 
    local Player = CTRPFW.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		TriggerClientEvent('ctrp-gunheist:useChip', source)

	end
end)

CTRPFW.Functions.CreateUseableItem("greenchip", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		local itemName = item.name
		TriggerClientEvent('ctrp-gunheist:useDecryptionKey', source,  'greenchip')
    end
end)

CTRPFW.Functions.CreateUseableItem("redchip", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		local itemName = item.name
		TriggerClientEvent('ctrp-gunheist:useDecryptionKey', source,  'redchip')
    end
end)

CTRPFW.Functions.CreateUseableItem("bluechip", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		local itemName = item.name
		TriggerClientEvent('ctrp-gunheist:useDecryptionKey', source, 'bluechip')
    end
end)

...
local hackingPosition = vector3(-60.09, -2518.03, 7.4)
...
RegisterNetEvent('ctrp-gunheist:manipulateDrop')
AddEventHandler('ctrp-gunheist:manipulateDrop', function(dropId, dropData)
...
RegisterNetEvent('ctrp-gunheist:obtainDrops')
AddEventHandler('ctrp-gunheist:obtainDrops', function(serverDrops)
	activeDrops = serverDrops
end)

RegisterNetEvent('ctrp-gunheist:createBlipOnDrop')
AddEventHandler('ctrp-gunheist:createBlipOnDrop', function(dropId)
...
RegisterNetEvent('ctrp-gunheist:openCrate')
AddEventHandler('ctrp-gunheist:openCrate', function(crateObject)
...
Citizen.CreateThread(function()
	TriggerServerEvent('ctrp-gunheist:obtainDrops')
...
AddEventHandler('do', function(dropId)
	...
	TriggerEvent('ctrp-gunheist:openCrate', activeDrops[dropId].object)
	
	-- local finished = exports["op-progressbar"]:progressBar("Drilling Crate", math.random(20000, 30000))
	CTRPFW.Functions.Progressbar("drilling_", "Drilling...", math.random(90000, 120000), false, true, {}, {}, {}, {}, function()
		TriggerServerEvent('ctrp-gunheist:pickUpDrop', dropId)
		inAnimation = false
	end)
end) 
...
RegisterNetEvent('ctrp-gunheist:useChip')

AddEventHandler('ctrp-gunheist:useChip', function()
	local dstcheck = #(GetEntityCoords(PlayerPedId()) - hackingPosition)

	if dstcheck < 5 then
		if StartHacking() then
			TriggerServerEvent('ctrp-gunheist:wonHacking')
		end
	end
end)
...
AddEventHandler('ctrp-gunheist:doDecryptionAnimation', function()
	local ped = PlayerPedId()
	RequestAnimDict('missheist_jewel@hacking')
	TaskPlayAnim(ped, "missheist_jewel@hacking", "hack_loop", 8.0, 1.0, -1, 9, 0, 0, 0, 0)
end)

RegisterNetEvent('ctrp-gunheist:useDecryptionKey')
AddEventHandler('ctrp-gunheist:useDecryptionKey', function(item)
	local dstcheck = #(GetEntityCoords(PlayerPedId()) - hackingPosition)
	-- print(item)
	if dstcheck < 5 then
		TriggerEvent('ctrp-gunheist:doDecryptionAnimation')


		CTRPFW.Functions.Progressbar("drilling_", "Decrypting with key...", math.random(10000, 15000), false, true, {}, {}, {}, {}, function()
			ClearPedTasks(PlayerPedId())
			RemoveAnimDict("missheist_jewel@hacking")
			TriggerServerEvent('ctrp-gunheist:server:useDecryptionKey', item)
		end)
	end
end)
...
function hackingCompleted(success, timeremaining)
	if success then	
		TriggerServerEvent('ctrp-gunheist:hackingCompleted')
	end

	TriggerEvent('mhackings:hide')
end

RegisterNetEvent('ctrp-gunheist:hackingMinigame')
AddEventHandler('ctrp-gunheist:hackingMinigame', function()
	TriggerEvent("mhackings:show")
	TriggerEvent("mhackings:start", 7, 35, hackingCompleted)
end)

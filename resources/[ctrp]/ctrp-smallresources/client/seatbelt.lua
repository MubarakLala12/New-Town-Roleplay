    ...
    while true do
        ...
        if IsControlJustReleased(0, Keys["B"]) then 
            if not harnessOn then
                ...
                CTRPFW.Functions.Progressbar("harness_equip", "Removing Harness..", 5000, false, true, {
    ...
    while true do
        ...
        if currentVehicle ~= nil and currentVehicle ~= false and currentVehicle ~= 0 then
            ...
            if frameBodyChange ~= 0 then
                if lastFrameVehiclespeed > 200 and thisFrameVehicleSpeed < (lastFrameVehiclespeed * 0.75) and not damagedone then
            ...
            if lastVehicle ~= nil then
                ...
                if not damagedone and newvehicleBodyHealth < currentvehicleBodyHealth then
                    damagedone = true
                    SetVehicleEngineHealth(lastVehicle, engineHealth - math.random(130, 160))
...
function ToggleHarness(toggle)
    ...
    TriggerEvent('ctrp-hud:client:ToggleHarness', toggle)
end
...
AddEventHandler('seatbelt:client:UseHarness', function(ItemData)
    ...
    if inveh and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
        if not harnessOn then
            CTRPFW.Functions.Progressbar("harness_equip", "Putting on Racing Harness..", 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        CTRPFW.Functions.Notify('You\'re not in a car.', 'error', 3500)
    end
end)

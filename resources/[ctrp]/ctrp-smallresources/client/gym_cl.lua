...
function startExercise(animInfo, pos)
    ...
    Citizen.CreateThread(function()
        while exercising do
            ...
            DrawText2D(0.505, 0.95, 1.0,1.0,0.33, "Press ~g~[SPACE]~w~ to start exercising", 255, 255, 255, 255)
        ...
        while canExercise do
            ...
            if procent <= 99 then
                ...
                if IsControlJustPressed(0, Keys["SPACE"]) then -- press space to train
                    ...
                    canExercise = true
                    TriggerServerEvent('virus-hudcomponent:Server:RelieveStress', math.random(2, 5))
                end
    ...
    Citizen.CreateThread(function()
        while doingMotion do
            ...
            if motionProcent >= 100000 then
                doingMotion = false
                motionProcent = 0
                Notify("You ~g~finished~w~ your exercise session.")
            end
        end
    end)

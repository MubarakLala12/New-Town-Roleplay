...
$(document).on('click', '.weapon-attachments-back', function(e){
    e.preventDefault();
    $("#ctrp-inventory").css({"display":"block"});
    $("#ctrp-inventory").animate({
        left: 0+"vw"
    }, 200);
...
function FormatAttachmentInfo(data) {
    $.post("https://ctrp-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('https://ctrp-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#ctrp-inventory").animate({
            ...
            $("#ctrp-inventory").css({"display":"none"})
        });
        $(".weapon-attachments-container").animate({
            left: 0+"vw"
        }, 200);
        ...
        $.post('https://ctrp-inventory/Notify', JSON.stringify({
    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            if (itemData.info.type == "casing") {
                ...
                $(".item-info-description").html('<p><strong>Evidence material: </strong><span>' + itemData.info.label + '</span></p><p><strong>Finger pattern: </strong><span>' + itemData.info.fingerprint + '</span></p><p><strong>Crime scene: </strong><span>' + itemData.info.street + '</span></p><br /><p>' + itemData.description + '</p>');
...
function handleDragDrop() {
    $(".item-drag").draggable({
        ...
        start: function(event, ui) {
            IsDragging = true;
           // $(this).css("background", "rgba(10,168,219,1.0)");
            $(this).find("img").css("filter", "brightness(50%)");

            $(".item-slot").css("border", "1px solid rgba(0, 42, 59,0.671)")

            var itemData = $(this).data("item");
            var dragAmount = $("#item-amount").val();
            if (!itemData.useable) {
                $("#item-use").css("background", "rgba(0, 42, 59,0.671");
            }
        ...
        stop: function() {
            ...
            $(this).css("background", "rgba(0, 42, 59,0.671)");
            $(this).find("img").css("filter", "brightness(100%)");
            $("#item-use").css("background", "rgba("+InventoryOption+", 0.671)");
        },
    });
    ...
    $("#item-use").droppable({
        ...
        drop: function(event, ui) {
            ...
            if(fromData.useable) {
                ...
                $.post("https://ctrp-inventory/UseItem", JSON.stringify({
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $(this).css("background", "rgba(0, 42, 59,0.671");
            $.post("https://ctrp-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
...
$(document).on('click', '.CombineItem', function(e){
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('https://ctrp-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('https://ctrp-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("https://ctrp-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {       
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("https://ctrp-inventory/PlayDropSound", JSON.stringify({}));
            $.post("https://ctrp-inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    $.post('https://ctrp-inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
...
function InventoryError($elinv, $elslot) {
    $elinv.find("[data-slot=" + $elslot + "]").css("background", "rgba(0, 42, 59,0.671)").css("transition", "background 500ms");
    setTimeout(function() {
        $elinv.find("[data-slot=" + $elslot + "]").css("background", "rgba(0, 42, 59,0.671)");
    }, 500)
    $.post("https://ctrp-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        if (data.other != null && data.other != "") {
            ...
            $(".other-inventory .item-slot").css({
                "background-color": "rgba(0, 42, 59,0.671)"
            });
        }
    ...
    Inventory.Close = function() {
        ...
        $("#ctrp-inventory").fadeOut(300);
        $(".combine-option-container").hide();
        $(".item-slot").remove();
        if ($("#rob-money").length) {
            $("#rob-money").remove();
        }
        $.post("https://ctrp-inventory/CloseInventory", JSON.stringify({}));

        if (AttachmentScreenActive) {
            $("#ctrp-inventory").css({"left": "0vw"});
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('https://ctrp-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});
...
AddEventHandler('CTRPFW:Client:OnPlayerLoaded', function()
    CTRPFW.Functions.TriggerCallback('ctrp-drugs:server:RequestConfig', function(DealerConfig)
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        CTRPFW.Functions.Progressbar("hospital_revive", "Put person back on their feet..", 5000, false, true, {
                                            ...
                                            CTRPFW.Functions.Notify("You have helped a person!")
                                        ...
                                        CTRPFW.Functions.Notify("There is no one nearby..", "error")
                                    end
                                else
...
function buyDealerStuff()
    ...
    for k, v in pairs(Config.Dealers[currentDealer]["products"]) do
        if CTRPFW.Functions.GetPlayerData().metadata["dealerrep"] >= Config.Dealers[currentDealer]["products"][k].minrep then
...
function knockDoorAnim(home)
    ...
    local myData = CTRPFW.Functions.GetPlayerData()
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        CTRPFW.Functions.Notify('It looks like no one is home..', 'error', 3500)
    end
end

RegisterNetEvent('ctrp-drugs:client:updateDealerItems')
AddEventHandler('ctrp-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('ctrp-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('ctrp-drugs:client:setDealerItems')
AddEventHandler('ctrp-drugs:client:setDealerItems', function(itemData, amount, dealer)
...
function requestDelivery()
    ...
    TriggerServerEvent('ctrp-drugs:server:giveDeliveryItems', amount)
    SetTimeout(2000, function()
        TriggerServerEvent('ctrp-phone:server:sendNewMail', {
            sender = Config.Dealers[currentDealer]["name"],
            subject = "Aflever Locatie",
            message = "Here is all info about the delivery, <br>Items: <br> "..amount.."x "..CTRPFW.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> be in time!!",
            button = {
                enabled = true,
                buttonEvent = "ctrp-drugs:client:setLocation",
...
function randomDeliveryItemOnRep()
    local ped = PlayerPedId()
    local myRep = CTRPFW.Functions.GetPlayerData().metadata["dealerrep"]
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    CTRPFW.Functions.Notify('The route to the delivery location has been set on your map.', 'success');
end

RegisterNetEvent('ctrp-drugs:client:setLocation')
AddEventHandler('ctrp-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        activeDelivery = locationData
    else
        setMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
        CTRPFW.Functions.Notify('You still have an open delivery...')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '[E] '..activeDelivery["amount"]..'x '..CTRPFW.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' deliver.')
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        checkPedDistance()
        CTRPFW.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent('ctrp-drugs:server:succesDelivery', activeDelivery, true)
        end, function() -- Cancel
            ClearPedTasks(PlayerPedId())
            CTRPFW.Functions.Notify("Canceled..", "error")
        end)
    else
        TriggerServerEvent('ctrp-drugs:server:succesDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
function doPoliceAlert()
    ...
    TriggerServerEvent('ctrp-drugs:server:callCops', streetLabel, pos)
end

RegisterNetEvent('ctrp-drugs:client:robberyCall')
AddEventHandler('ctrp-drugs:client:robberyCall', function(msg, streetLabel, coords)
...
RegisterNetEvent('ctrp-drugs:client:sendDeliveryMail')
AddEventHandler('ctrp-drugs:client:sendDeliveryMail', function(type, deliveryData)
...
RegisterNetEvent('ctrp-drugs:client:CreateDealer')
AddEventHandler('ctrp-drugs:client:CreateDealer', function(dealerName, minTime, maxTime)
    local ped = PlayerPedId()
    ...
    TriggerServerEvent('ctrp-drugs:server:CreateDealer', DealerData)
end)

RegisterNetEvent('ctrp-drugs:client:RefreshDealers')
AddEventHandler('ctrp-drugs:client:RefreshDealers', function(DealerData)
    Config.Dealers = DealerData
end)

RegisterNetEvent('ctrp-drugs:client:GotoDealer')
AddEventHandler('ctrp-drugs:client:GotoDealer', function(DealerData)
    local ped = PlayerPedId()

    SetEntityCoords(ped, DealerData["coords"]["x"], DealerData["coords"]["y"], DealerData["coords"]["z"])
    CTRPFW.Functions.Notify('You have been teleported : '.. DealerData["name"] .. ' Good luck!', 'success')
end)
...
RegisterNetEvent('ctrp-drugs:client:cornerselling')
AddEventHandler('ctrp-drugs:client:cornerselling', function(data)
    CTRPFW.Functions.TriggerCallback('ctrp-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            availableDrugs = result

            if not cornerselling then
                cornerselling = true
                CTRPFW.Functions.Notify('Flicked the cornerselling switch.. yeet')
                startLocation = GetEntityCoords(PlayerPedId())
                -- TaskStartScenarioInPlace(PlayerPedId(), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
            else
                cornerselling = false
                CTRPFW.Functions.Notify('Corner selling position disabled .. yeet')
                -- ClearPedTasks(PlayerPedId())
            end
        else
            CTRPFW.Functions.Notify('You aren\'t carrying any weed with you..', 'error')
        end
    end)
end)

function toFarAway()
    CTRPFW.Functions.Notify('You are moving too much, start over again!', 'error')
    cornerselling = false
    hasTarget = false
    busySelling = false
    startLocation = nil
            ...
            if IsEntityDead(stealingPed) then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, pedpos.x, pedpos.y, pedpos.z, true) < 1.5) then
                    ...
                    if IsControlJustReleased(0, Keys["E"]) then
                        ...
                        Citizen.Wait(2000)
                        ClearPedTasks(PlayerPedId())
                        TriggerServerEvent("CTRPFW:Server:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', CTRPFW.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
...
RegisterNetEvent('ctrp-drugs:client:refreshAvailableDrugs')
AddEventHandler('ctrp-drugs:client:refreshAvailableDrugs', function(items)
...
function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('ctrp-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                CTRPFW.Functions.Notify('You have been robbed and lost '..bagAmount..' bags(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 then
                    CTRPFW.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' for $'..randomPrice..'? / ~g~G~w~ Decline offer')
                    if IsControlJustPressed(0, Keys["E"]) then
                        CTRPFW.Functions.Notify('Offer accepted!', 'success')
                        TriggerServerEvent('ctrp-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, Keys["G"]) then
                        CTRPFW.Functions.Notify('Offer canceled!', 'error')
                        hasTarget = false

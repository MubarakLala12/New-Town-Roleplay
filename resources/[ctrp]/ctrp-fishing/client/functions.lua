TryToFish = function()
    CTRPFW.Functions.TriggerCallback('ctrp-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return CTRPFW.Functions.Notify("You can't be swimming and fishing at the same time.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return CTRPFW.Functions.Notify("You need to exit your vehicle to start fishing.", "error") end 
        if count ~= nil then
            if count == 0 then
                CTRPFW.Functions.Notify("You need both a fishing rod and bait to fish.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    CTRPFW.Functions.Notify("You need to aim towards the water to fish", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        CTRPFW.Functions.Notify('You don\'t have any bait!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            CTRPFW.Functions.Notify("You need to cast the bait.", "primary")
    ...
    CTRPFW.Functions.Notify("Waiting for a fish to bite...", "success", "10000")
    TriggerServerEvent('CTRPFW:Server:RemoveItem', "fishingbait", 1)
	TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["fishingbait"], "remove")
    ...
    if caughtFish then
        giveFish()
        TriggerServerEvent('virus-hudcomponent:Server:RelieveStress', 5)
    else
        CTRPFW.Functions.Notify("The fish got loose.", "error")
    end
...
IsInWater = function()
    ...
    CTRPFW.Functions.Notify("Checking fishing location...", "success", "3000")
...
SellFish = function()
    CTRPFW.Functions.TriggerCallback('ctrp-fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("ctrp-fishing:sellFish", function(sold, fishesSold) end)
    end)
end
...
ShowHelpNotification = function(msg, thisFrame, beep, duration)
	AddTextEntry('ctrpHelpNotification', msg)

	if thisFrame then
		DisplayHelpTextThisFrame('ctrpHelpNotification', false)
	else
		if beep == nil then beep = true end
		BeginTextCommandDisplayHelp('ctrpHelpNotification')
...
function HasFishingBait()
    local rtval = false
    CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(amount)
        if amount then
            rtval = true
        end
    end, "fishingbait")
    ...
    if kismatFound then
        for i = 1, kismatCount, 1 do
            if kismat == 100 then
                ...
                TriggerServerEvent("ctrp-fishing:receiveFish", castLocation, function(received) end)

            elseif kismat >= 90 and kismat <= 99 then
				TriggerServerEvent("ctrp-fishing:receiveFish", castLocation, function(received) end)

            elseif kismat >= 80 and kismat <= 90 then
				TriggerServerEvent("CTRPFW:Server:RemoveItem", "fishingrod", 1)
                TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["fishingrod"], "remove")
                CTRPFW.Functions.Notify("Opps! Your rod got broken!", "error")
                interupted = true
            ...
            elseif kismat >= 50 and kismat <= 60 then
				TriggerServerEvent("CTRPFW:Server:RemoveItem", "fishingrod", 1)
                TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["fishingrod"], "remove")
                CTRPFW.Functions.Notify("Opps! Your rod got broken!", "error")
                interupted = true

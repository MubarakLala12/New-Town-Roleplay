...
CTRPFW.Functions.CreateCallback('ctrp-fishing:GetItemData', function(source, cb, itemName)
	local retval = false
...
CTRPFW.Functions.CreateUseableItem("fishingrod", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)

    TriggerClientEvent('ctrp-fishing:tryToFish', source)
end)

--[[RegisterServerEvent('ctrp-fishing:receiveFish')
AddEventHandler('ctrp-fishing:receiveFish', function(cabin, house)
    local src = source
...
RegisterServerEvent("ctrp-fishing:sellFish")
AddEventHandler("ctrp-fishing:sellFish", function()
    local src = source
	local Player = CTRPFW.Functions.GetPlayer(src)
	local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
		...
		TriggerClientEvent('CTRPFW:Notify', src, "You have sold your fish")
	end
end)
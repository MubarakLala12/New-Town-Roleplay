	...
	if Player.PlayerData.item ~= nil and next(Player.PlayerData.items) ~= nil then
		for k, v in pairs(Player.PlayerData.items) do
		    if Player.Playerdata.items[k] ~= nil then
				if Player.Playerdata.items[k].name == "coke_brick" and Player.Playerdata.items[k].name == "bakingsoda" and Player.Playerdata.items[k].name == "highqualityscales"  then
					cb(true)
			    else
...
CTRPFW.Functions.CreateCallback('coke:ingredient', function(source, cb)
    ...
    if coke_brick ~= nil and bakingsoda ~= nil and highqualityscales ~= nil and id_card ~= nil then
        cb(true)
    else
        cb(false)
    end

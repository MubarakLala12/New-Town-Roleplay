...
AddEventHandler("CTRPFW:Client:OnPlayerLoaded", function()
    CTRPFW.Functions.TriggerCallback('ctrp-tacos:server:GetConfig', function(config)
        Config = config
    end)
end)
				 ...
				 if IsControlJustPressed(0, Keys['E']) then
				  if not Bezig then
					...
					elseif Config.JobData['locations'][k]['name'] == 'Meat' then
						BakeMeat()
					elseif Config.JobData['locations'][k]['name'] == 'Shell' then
						CTRPFW.Functions.TriggerCallback('ctrp-taco:server:get:ingredient', function(HasItems)  
                        if HasItems then
							FinishTaco()
						else
							CTRPFW.Functions.Notify("You don\'t have all the Ingredients yet.", "error")
						end
					end)
					elseif Config.JobData['locations'][k]['name'] == 'Register' then
						TakeMoney()
					...
					elseif Config.JobData['locations'][k]['name'] == 'GiveTaco' then
						...
						CTRPFW.Functions.Notify("You're still doing something man bro.", "error")
					end
				end
			end
		end
...
function FinishTaco()
	...
	CTRPFW.Functions.Progressbar("pickup_sla", "Making taco..", 3500, false, true, {
		...
		TriggerServerEvent('CTRPFW:Server:RemoveItem', "meat", 1)
		TriggerServerEvent('CTRPFW:Server:RemoveItem', "lettuce", 1)
		TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["meat"], "remove")
		TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["lettuce"], "remove")
		TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["taco"], "add")
		TriggerServerEvent('CTRPFW:Server:AddItem', "taco", 1)
...
function BakeMeat()
	...
	CTRPFW.Functions.Progressbar("pickup_sla", "Baking meat..", 5000, false, true, {
		disableMovement = true,
		disableCarMovement = false,
		disableMouse = false,
		disableCombat = false,
		...
		TriggerServerEvent('CTRPFW:Server:AddItem', "meat", 1)
		Config.JobData['stock-meat']= Config.JobData['stock-meat'] - 1
		TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["meat"], "add")
		Bezig = false
	end, function()
		CTRPFW.Functions.Notify("Canceled..", "error")
		Bezig = false
	...
	CTRPFW.Functions.Notify("There is not enough meat in stock.", "error")
 end  
end
...
function GetLettuce()
	...
	CTRPFW.Functions.Progressbar("pickup_sla", "Grabbing lettuce..", 4100, false, true, {
		disableMovement = true,
		disableCarMovement = false,
		disableMouse = false,
		disableCombat = false,
		...
		TriggerServerEvent('CTRPFW:Server:AddItem', "lettuce", 1)
		Config.JobData['stock-lettuce']= Config.JobData['stock-lettuce'] - 1
		TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["lettuce"], "add")
		Bezig = false
	end, function()
		StopAnimTask(GetPlayerPed(-1), "amb@prop_human_bum_bin@idle_b", "idle_d", 1.0)
		CTRPFW.Functions.Notify("Canceled..", "error")
	...
	CTRPFW.Functions.Notify("There is not enough lettuce in stock.", "error")
 end 
end

function GiveTacoToShop()
	CTRPFW.Functions.TriggerCallback('ctrp-taco:server:get:tacos', function(HasItem, type)
		if HasItem then
		  if not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
			if Config.JobData['tacos'] <= 9 then	
				CTRPFW.Functions.Notify("Taco delivered!", "success")
				Config.JobData['tacos'] = Config.JobData['tacos'] + 1
				TriggerServerEvent('CTRPFW:Server:RemoveItem', "taco", 1)
				TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["taco"], "remove")
				else
					CTRPFW.Functions.Notify("There are still 10 taco\'s that have to be sold. We dont waste food here..", "error")
				end
		  elseif type == 'green' then
			if Config.JobData['green-tacos'] <= 9 then	
				TriggerServerEvent('CTRPFW:Server:RemoveItem', "taco", 1)
				TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["green-taco"], "remove")
				else
					CTRPFW.Functions.Notify("There are still 10 taco\'s that have to be sold. We dont waste food here..", "error")
				end
		end
	    else
		CTRPFW.Functions.Notify("You dont even have a taco.", "error")
	 end
	end)
end

function AddStuff()
	CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(HasItem)
		if HasItem then
			if Config.JobBusy == true then
				TriggerServerEvent('CTRPFW:Server:RemoveItem', "taco-box", 1)
				TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items["taco-box"], "remove")
				...
				CTRPFW.Functions.Notify("Taco Shop has been restocked!", "success")
				Config.JobBusy = false
			else
				CTRPFW.Functions.Notify("You\'re coming straight from the taco store.", "error")
			end
		else
			CTRPFW.Functions.Notify("You don\'t even have a box of ingredients.", "error")
		end
	end, 'taco-box')
end
...
function TakeMoney()
	if Config.JobData['register'] >= 10000 then
		...
		CTRPFW.Functions.Progressbar("search_register", "Empty cash register..", lockpickTime, false, true, {
            disableMovement = true,
		...
		CTRPFW.Functions.Notify("There is not enough money in the register yet.", "error")
	end
end
...
function RegisterAnim(time)
	...
	while GetMoney do
		...
		Citizen.Wait(2000)
		time = time - 2
		TriggerServerEvent('ctrp-storerobbery:server:takeMoney', currentRegister, false)
		if time <= 0 then
			GetMoney = false

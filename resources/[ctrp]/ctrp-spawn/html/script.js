...
$(document).on('click', '.location', function(evt){
    ...
    if (type !== "lab") {
        ...
        $.post('http://ctrp-spawn/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
...
$(document).on('click', '#submit-spawn', function(evt){
    ...
    if (spawnType !== "appartment") {
        $.post('http://ctrp-spawn/spawnplayer', JSON.stringify({
            spawnloc: location,
            typeLoc: spawnType
        }));
    } else {
        $.post('http://ctrp-spawn/chooseAppa', JSON.stringify({
            appType: location,
        }));
    }
});
...
function setupLocations(locations, myHouses) {
    ...
    setTimeout(function(){
        ...
        $(parent).append('<div class="submit-spawn" id="submit-spawn"><p> <img id="joyzzchutiya" src="https://cdn.discordapp.com/attachments/699953862706200577/876062636045893633/vlogo2.png"></p></div>');
        $('.submit-spawn').hide();
    }, 100)
}

$(document).keydown(function(objEvent) {
    if (objEvent.keyCode == 84) {  //T pressed

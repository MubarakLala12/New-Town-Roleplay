				...
				if GetClockHours() >= 1 and GetClockHours() <= 4 then
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						...
						if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            CTRPFW.Functions.Progressbar("sell_pawn_items", "Selling stuff ..", math.random(15000, 25000), false, true, {}, {}, {}, {}, function() -- Done
                                ClearPedTasks(GetPlayerPed(-1))
								TriggerServerEvent("ctrp-pawnshop:server:sellPawnItems")
								sellItemsSet = false
								sellPrice = 0
                            end, function() -- Cancel
								ClearPedTasks(GetPlayerPed(-1))
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.PawnHardwareLocation.x, Config.PawnHardwareLocation.y, Config.PawnHardwareLocation.z, true) < 1.5 then
				if GetClockHours() >= 9 and GetClockHours() <= 16 then
					...
					elseif sellHardwareItemsSet and sellHardwarePrice ~= 0 then
						...
						if IsControlJustReleased(0, Keys["E"]) then
							TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_STAND_IMPATIENT", 0, true)
                            CTRPFW.Functions.Progressbar("sell_pawn_items", "Selling ..", math.random(15000, 25000), false, true, {}, {}, {}, {}, function() -- Done
                                ClearPedTasks(GetPlayerPed(-1))
								TriggerServerEvent("ctrp-pawnshop:server:sellHardwarePawnItems")
...
function GetSellingPrice()
	local price = 0
	CTRPFW.Functions.TriggerCallback('ctrp-pawnshop:server:getSellPrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price
...
function GetSellingHardwarePrice()
	local price = 0
	CTRPFW.Functions.TriggerCallback('ctrp-pawnshop:server:getSellHardwarePrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price

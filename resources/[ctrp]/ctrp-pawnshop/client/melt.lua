...
AddEventHandler("CTRPFW:Client:OnPlayerLoaded", function()
    CTRPFW.Functions.TriggerCallback('ctrp-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("ctrp-pawnshop:server:getGoldBars")
                        end
                    else
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then 
                            ...
                            ScrapAnim(1000)
                            CTRPFW.Functions.Progressbar("drop_golden_stuff", "Picking up items..", 1000, false, true, {
                                ...
                                if not Config.IsMelting then
                                    ...
                                    TriggerServerEvent("ctrp-pawnshop:server:meltItems")
                                end
                            end)
                        end
                    end
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            CTRPFW.Functions.Progressbar("sell_gold", "Sell gold..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                TriggerServerEvent('ctrp-pawnshop:server:sellGold')
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(GetPlayerPed(-1))
...
function HasPlayerGold()
	local retval = false
	CTRPFW.Functions.TriggerCallback('ctrp-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('ctrp-pawnshop:client:startMelting')
AddEventHandler('ctrp-pawnshop:client:startMelting', function()
    if not Config.IsMelting then
...
RegisterNetEvent('ctrp-pawnshop:client:SetTakeState')
AddEventHandler('ctrp-pawnshop:client:SetTakeState', function(state)

...
local jaanado2 = false

RegisterNetEvent('vehiclekeys:client:allowed')
AddEventHandler('vehiclekeys:client:allowed', function(jaanado)
    jaanado2 = jaanado
end)

RegisterNetEvent('vehiclekeys:client:notallowed')
AddEventHandler('vehiclekeys:client:notallowed', function(jaanado)
    jaanado2 = jaanado
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if CTRPFW ~= nil then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), true), -1) == GetPlayerPed(-1) then
                ...
                if LastVehicle ~= GetVehiclePedIsIn(GetPlayerPed(-1), false) then
                    CTRPFW.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
                        if result then
                            HasKey = true
        ...
        if not HasKey and IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and CTRPFW ~= nil and not IsHotwiring and not jaanado2 then
            local veh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
            SetVehicleEngineOn(veh, false, false, true)
            local veh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
            local vehpos = GetOffsetFromEntityInWorldCoords(veh, 0, 1.5, 0.5)
            CTRPFW.Functions.DrawText3D(vehpos.x, vehpos.y, vehpos.z, "~g~H~w~ - Hotwire  |  ~g~G~w~ - Search")
            SetVehicleEngineOn(veh, false, false, true)
      ...
      if aiming then
       if DoesEntityExist(targetPed) and not IsPedAPlayer(targetPed) and IsPedArmed(PlayerPedId(), 7) then
        ...
        if not IsEntityDead(targetPed) and distance < 12 then
         ...
         CTRPFW.Functions.Notify('You have been handed the keys!', 'success')
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(7)
        if not IsRobbing and isLoggedIn and CTRPFW ~= nil then
            if GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= nil and GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= 0 then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) then
                    if IsEntityDead(driver) then
                        IsRobbing = true
                        CTRPFW.Functions.Progressbar("rob_keys", "Grabing keys..", 3000, false, true, {}, {}, {}, {}, function() -- Done
                            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
                            HasKey = true
                            IsRobbing = false
                        end)
            ...
            --[[if CTRPFW.Functions.GetPlayerData().job.name ~= "police" then
...
AddEventHandler('vehiclekeys:client:SetOwner', function(plate)
    ...
    HasKey = true
    --CTRPFW.Functions.Notify('You picked the keys of the vehicle', 'success', 3500)
end)
...
AddEventHandler('vehiclekeys:client:GiveKeys', function(target)
    local plate = GetVehicleNumberPlateText(GetVehiclePedIsIn(GetPlayerPed(-1), true))
    if jaanado2 then
        CTRPFW.Functions.Notify("Can't share test vehicle key")
    else
        TriggerServerEvent('vehiclekeys:server:GiveVehicleKeys', plate, target)
...
AddEventHandler('lockpicks:UseLockpick', function(isAdvanced)
    if (IsPedInAnyVehicle(GetPlayerPed(-1))) then
        if not HasKey then
            ...
            print("afsos wo cheej nikal gyi")
        end
    else
...
function LockVehicle()
    local veh = CTRPFW.Functions.GetClosestVehicle()
    ...
    if veh ~= nil and GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 2.0 then
        CTRPFW.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
            if result then
                if HasKey then
                    ...
                    if vehLockStatus == 1 then
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 2)then
                            CTRPFW.Functions.Notify("Vehicle locked!")
                        else
                            CTRPFW.Functions.Notify("Something went wrong whit the locking system!")
                        end
                    else
                        Citizen.Wait(450)
                        ClearPedTasks(GetPlayerPed(-1))
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 1)then
                            CTRPFW.Functions.Notify("Vehicle unlocked!")
                        else
                            CTRPFW.Functions.Notify("Something went wrong whit the locking system!")
                        end
                    end
                ...
                CTRPFW.Functions.Notify('You dont have the keys of the vehicle..', 'error')
            end
        end, plate)
    end
end
    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 1.5 then
            ...
            if (vehLockStatus > 1) then
                ...
                CTRPFW.Functions.Progressbar("lockpick_vehicledoor", "breaking the door open..", lockpickTime, false, true, {
                    ...
                    if math.random(1, 100) <= 90 then
                        CTRPFW.Functions.Notify("Door open!")
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "unlock", 0.3)
                ...
                Skillbar.Start({
                    ...
                    if IsHotwiring then
                        if SucceededAttempts + 1 >= NeededAttempts then
                            ...
                            CTRPFW.Functions.Notify("Lockpicking succeeded!")
                            HasKey = true
                            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
                            IsHotwiring = false
                            FailedAttemps = 0
                            SucceededAttempts = 0
                            NeededAttempts = 0
                            TriggerServerEvent('virus-hudcomponent:Server:GainStress', math.random(2, 4))
                        else
                            if vehicle ~= nil and vehicle ~= 0 then
                                ...
                                CTRPFW.Functions.Notify("You have to be in the vehicle", "error")
                                IsHotwiring = false
                                FailedAttemps = FailedAttemps + 1
                                local c = math.random(2)
                                local o = math.random(2)
                                if c == o then
                                    TriggerServerEvent('virus-hudcomponent:Server:GainStress', math.random(1, 4))
                                end
                            end
                        end
                    end
                    ...
                    if IsHotwiring then
                        ...
                        HasKey = false
                        SetVehicleEngineOn(vehicle, false, false, true)
                        CTRPFW.Functions.Notify("Lockpicking failed!", "error")
                        IsHotwiring = false
                        FailedAttemps = FailedAttemps + 1
                        local c = math.random(2)
                        local o = math.random(2)
...
function Hotwire()
    if not HasKey then 
        ...
        PoliceCall()
        CTRPFW.Functions.Progressbar("hotwire_vehicle", "Engaging the ignition switch", hotwireTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            if (math.random(0, 100) < 40) then
                HasKey = true
                CTRPFW.Functions.Notify("Hotwire succeeded!")
            else
                HasKey = false
                SetVehicleEngineOn(veh, false, false, true)
                CTRPFW.Functions.Notify("Hotwire failed!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
...
function SearchHotwire()
    if not HasKey then 
        ...
        SetVehicleAlarmTimeLeft(vehicle, hotwireTime)
        PoliceCall()
        CTRPFW.Functions.Progressbar("search_vehicle", "Searching", hotwireTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
            TriggerServerEvent("ctrp-vehiclekeys:server:searchVirus", Config.VehicleItems)
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
           
            CTRPFW.Functions.Notify("Search failed!", "error")
            IsHotwiring = false
        end)
    end
end
...
RegisterNetEvent('vehiclekeys:client:doneVirus')
AddEventHandler('vehiclekeys:client:doneVirus', function()
    HasKey = true
    CTRPFW.Functions.Notify("Found Key!")
end)
        ...
        if math.random(1, 100) <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    if CTRPFW.Shared.VehicleModels[modelName] ~= nil then
                        Name = CTRPFW.Shared.Vehicles[CTRPFW.Shared.VehicleModels[modelName]["model"]]["brand"] .. ' ' .. CTRPFW.Shared.Vehicles[CTRPFW.Shared.VehicleModels[modelName]["model"]]["name"]
                    else
                        Name = "Unknown"
                    end

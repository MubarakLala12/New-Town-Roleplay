...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.action == "sellVehicle") {
            ...
            CTRPOccasions.setupSellContract(eventData)
        }

        if (eventData.action == "buyVehicle") {
            $('.buy-container').fadeIn(150);
            CTRPOccasions.setupBuyContract(eventData, eventData.vehicleData)
        }
    });
});
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27:
            ...
            $.post('http://ctrp-occasions/close');
            break;
    }
});
...
$(document).on('click', '#sell-vehicle', function(){
    if ($('.vehicle-sell-price').val() != "") {
        if (!isNaN($('.vehicle-sell-price').val())) {
            $.post('http://ctrp-occasions/sellVehicle', JSON.stringify({
            ...
            $.post('http://ctrp-occasions/error', JSON.stringify({
...
$(document).on('click', '#buy-vehicle', function(){
    $.post('http://ctrp-occasions/buyVehicle');
    $('.buy-container').fadeOut(100);
    $.post('http://ctrp-occasions/close');
});

CTRPOccasions.setupSellContract = function(data) {
...
CTRPOccasions.setupBuyContract = function(data, vData) {

...
$(document).on('click', '#save', function(){
    $.post('http://ctrp-tunerchip/save', JSON.stringify({
...
$(document).on('click', '#reset', function(){
    $.post('http://ctrp-tunerchip/reset');
});
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('http://ctrp-tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('http://ctrp-tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('http://ctrp-tunerchip/SetStancer', JSON.stringify({
...
CTRPTuner.Open = function() {
    $.post('http://ctrp-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
CTRPTuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('http://ctrp-tunerchip/exit');
}
...
RegisterNUICallback('save', function(data)
    CTRPFW.Functions.TriggerCallback('ctrp-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = GetPlayerPed(-1)
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            CTRPFW.Functions.Notify('OutcastTuning v1.05: Vehicle adapted!', 'error')

            TriggerServerEvent('ctrp-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('ctrp-tunerchip:server:TuneStatus')
AddEventHandler('ctrp-tunerchip:server:TuneStatus', function()
    local ped = GetPlayerPed(-1)
    ...
    CTRPFW.Functions.TriggerCallback('ctrp-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    CTRPFW.Functions.Notify('OutcastTuning v1.05: Vehicle has been reset!', 'error')
end)

RegisterNetEvent('ctrp-tunerchip:client:openChip')
AddEventHandler('ctrp-tunerchip:client:openChip', function()
    local ped = GetPlayerPed(-1)
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        if PlayerJob.name == "mechanic" then 
            CTRPFW.Functions.Progressbar("connect_laptop", "Connecting..", 2000, false, true, {

...
AddEventHandler('CTRPFW:Client:OnPlayerLoaded', function()
    CTRPFW.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            CTRPFW.Functions.Progressbar("use_nos", "NOS is connected.", 1000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function() -- Done
                TriggerEvent("inventory:client:ItemBox", CTRPFW.Shared.Items['nitrous'], "remove")
                TriggerServerEvent("CTRPFW:Server:RemoveItem", 'nitrous', 1)
                local CurrentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
                local Plate = GetVehicleNumberPlateText(CurrentVehicle)
                TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
            end)
        else
            CTRPFW.Functions.Notify('You\'re not in a car.', 'error')
        end
    else
        CTRPFW.Functions.Notify('You already have NOS active..', 'error')
    end
end)
        ...
        if IsInVehicle then
            ...
            if VehicleNitrous[Plate] ~= nil then
                if VehicleNitrous[Plate].hasnitro then
                        ...
                        Citizen.CreateThread(function()
                            while NitrousActivated do
                                if VehicleNitrous[Plate].level - 1 ~= 0 then
                                    ...
                                    TriggerEvent('ctrp-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, true)
                                else
                    ...
                    if IsControlJustReleased(0, Keys["LEFTCTRL"]) then
                        if NitrousActivated then
                            ...
                            StopScreenEffect("RaceTurbo")
                            TriggerEvent('ctrp-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, false)
                            NitrousActivated = false
                        end
                    end
                end
                ...
                if not nosupdated then
                    TriggerEvent('ctrp-hud:client:UpdateNitrous', false, nil, false)
                    nosupdated = true
                end
            end
        else

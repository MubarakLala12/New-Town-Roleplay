...
function AddPoliceAlert(data) {
    ...
    if (data.alertTitle == "Assistance colleague" || data.alertTitle == "Ambulance assistance" || data.alertTitle == "Assistance Doctor") {
        $alert.css({
            "border-right-color": "#cc2c2c"
        });
    } else if (data.alertTitle == "Bank robbery attempt") {
        $alert.css({
            "border-right-color": "#cc2c2c"
        });
    } else if (data.alertTitle == "Prison outbreak") {
        $alert.css({
            "border-right-color": "#cc2c2c"
        });
    }
...
$(document).on('click', '.alert-location', function(e){
    ...
    $.post('http://ctrp-policealerts/SetWaypoint', JSON.stringify({
        coords: LocationData,
    }))
});
...
RegisterNetEvent('ctrp-policealerts:ToggleDuty')
AddEventHandler('ctrp-policealerts:ToggleDuty', function(Duty)
    PlayerJob.onduty = Duty
end)

RegisterNetEvent('ctrp-policealerts:client:AddPoliceAlert')
AddEventHandler('ctrp-policealerts:client:AddPoliceAlert', function(data, forBoth)
    if forBoth then
...
RegisterNUICallback('SetWaypoint', function(data)
    ...
    CTRPFW.Functions.Notify('GPS set!')
    SetNuiFocus(false, false)
    SetNuiFocusKeepInput(false, false)
    MouseActive = false
end)

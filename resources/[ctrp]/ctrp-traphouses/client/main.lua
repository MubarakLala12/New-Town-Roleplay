...
Citizen.CreateThread(function()
    ...
    if CTRPFW.Functions.GetPlayerData() ~= nil then
        ...
        CTRPFW.Functions.TriggerCallback('ctrp-traphouses:server:GetTraphousesData', function(trappies)
...
RegisterNetEvent('ctrp-traphouses:client:EnterTraphouse')
AddEventHandler('ctrp-traphouses:client:EnterTraphouse', function(code)
    if ClosestTraphouse ~= nil then
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        CTRPFW.Functions.Notify('This code is not valid..', 'error')
    end
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('ctrp-traphouses:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent('ctrp-traphouses:server:TakeoverHouse', CurrentTraphouse)
                            end
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, Keys["G"]) then
                                    CTRPFW.Functions.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, Keys["H"]) then
                                local TraphouseInventory = {}
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent("ctrp-traphouses:server:TakeMoney", CurrentTraphouse)
                            end
                        end
                    end
                end
...
function LeaveTraphouse(data)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    exports['ctrp-interior']:DespawnInterior(TraphouseObj, function()
...
RegisterNetEvent('ctrp-traphouses:client:TakeoverHouse')
AddEventHandler('ctrp-traphouses:client:TakeoverHouse', function(TraphouseId)
    local ped = GetPlayerPed(-1)

    CTRPFW.Functions.Progressbar("takeover_traphouse", "Overtaking traphouse", math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('ctrp-traphouses:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        CTRPFW.Functions.Notify("Overtaking canceled..", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            CTRPFW.Functions.Notify(CitizenId..' is added to the traphouse!')
        else
            CTRPFW.Functions.Notify(CitizenId..' This person does already have the keys!')
        end
    else
        CTRPFW.Functions.Notify('You can max give 6 other people acces to the trap house!')
    end
    IsKeyHolder = HasKey(CitizenId)
    IsHouseOwner = IsOwner(CitizenId)
end

RegisterNetEvent('ctrp-traphouses:client:SyncData')
AddEventHandler('ctrp-traphouses:client:SyncData', function(k, data)

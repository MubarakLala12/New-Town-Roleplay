...
-- CONFIG --
roleList = { 
	['👮 LSPD | '] = {"801920560602021918", 3, 'https://discord.com/api/webhooks/853596086546858006/OcsQ4IVJfqkqRmLaF5ZJvI77IMSjF2PN-OPVzLT6FwOxeQEEGQuP2Kp78SN0Sl9a6YPk'},
	['👨‍🚒 EMS | '] = {"812319221819375646", 1, 'https://discord.com/api/webhooks/853596338359631912/Kv2w_BTDC1gqyBjCxkDQYL406-xKLBEcSt2lCwrFE9YCioOiU0ni-rQG6SAE2zQRD4If'},
}
...
AddEventHandler("playerDropped", function()
	...
	if onDuty[source] ~= nil then 
		...
		if webHook ~= nil then 
			...
			sendToDisc('Player ' .. GetPlayerName(source) .. ' is now off duty', 'Player ' .. GetPlayerName(source) .. ' has gone off duty as ' .. tag.. '' ..Player.PlayerData.metadata["callsign"], 
	...
	if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "doctor" then 
		if hasPerms[source] ~= nil then 
			if onDuty[source] == nil then 
				...
				local webHook = roleList[activeBlip[source]][3];
				if webHook ~= nil then
					sendToDisc('Player ' .. GetPlayerName(source) .. ' is now on duty', 'Player ' .. GetPlayerName(source) .. ' has gone on duty as ' .. tag .. '' ..Player.PlayerData.metadata["callsign"], '',
						webHook, 65280)
				end
				TriggerEvent('eblips:add', {name = tag .. Player.PlayerData.metadata["callsign"], src = source, color = colorr}); 
				sendMsg(source, 'You have toggled your emergency blip ^2ON ^3and your Blip-Tag is: ' .. tag .. '' ..Player.PlayerData.metadata["callsign"])
				onDuty[source] = true;
			...
			sendMsg(source, '^1ERROR: You must have role on our discord to use this...')
		end
	else
...
-- 		TriggerClientEvent('chatMessage', source, '^9[^4' .. id .. '^9] ^0' .. Player.PlayerData.metadata["callsign"]);
-- 	end
-- end)
			...
			if tonumber(selection) ~= nil then 
				...
				if sel <= #theirBlips then
					...
					if onDuty[source] ~= nil then 
						...
						local minutesActive = os.difftime(now, startPlusNow) / (60);
						minutesActive = math.floor(math.abs(minutesActive))
						sendToDisc('Player ' .. GetPlayerName(source) .. ' is now off duty', 'Player ' .. GetPlayerName(source) .. ' has gone off duty as ' .. tag .. '' ..Player.PlayerData.metadata["callsign"], 'Duration: ' .. minutesActive,
							webHook, 16711680)
						timeTracker[source] = 0;
					end
			...
			for k, v in pairs(roleList) do
				for j = 1, #roleIDs do
					if exports.Badger_Discord_API:CheckEqual(v[1], roleIDs[j]) then
						...
						activeBlip[src] = k;
						hasPerms[src] = true;
						print("[CTRP]] Gave Perms Sucessfully")
					end
				end
			end

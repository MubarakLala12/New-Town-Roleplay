...
function CleanVehicle(vehicle)
	...
	CTRPFW.Functions.Progressbar("repair_vehicle", "Cleaning Vehicle...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		CTRPFW.Functions.Notify("Vehicle cleaned!")
		SetVehicleDirtLevel(vehicle, 0.1)
        SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		TriggerServerEvent('vehiclefailure:server:removewashingkit', vehicle)
		TriggerEvent('inventory:client:ItemBox', CTRPFW.Shared.Items["cleaningkit"], "remove")
...
function RepairVehicleFull(vehicle)
	...
	CTRPFW.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		CTRPFW.Functions.Notify("Vehicle repaired!")
		...
		TriggerServerEvent('ctrp-vehiclefailure:removeItem', "advancedrepairkit")
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		CTRPFW.Functions.Notify("Failed!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
function RepairVehicle(vehicle)
	...
	CTRPFW.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		if engineHealth == 1000.0 then
			CTRPFW.Functions.Notify("Engine Health is full" , "error")
		elseif engineHealth >= 900.0 then
			SetVehicleEngineHealth(vehicle, 1000.0)
			CTRPFW.Functions.Notify("Engine Health 100% now" , "success")
		elseif engineHealth >= 800.0 then
			SetVehicleEngineHealth(vehicle, 1000.0)
			CTRPFW.Functions.Notify("Engine Health 100% now" , "success")
		elseif engineHealth >= 700.0 then
			SetVehicleEngineHealth(vehicle, 1000.0)
			CTRPFW.Functions.Notify("Engine Health 100% now" , "success")
		elseif engineHealth >= 600.0 then
			SetVehicleEngineHealth(vehicle, 1000.0)
			CTRPFW.Functions.Notify("Engine Health 100% now" , "success")
		elseif engineHealth >= 500.0 then
			SetVehicleEngineHealth(vehicle, 1000.0)
			CTRPFW.Functions.Notify("Engine Health 100% now" , "success")
		elseif engineHealth >= 400.0 then
			SetVehicleEngineHealth(vehicle, 900.0)
			CTRPFW.Functions.Notify("Engine Health 90% now" , "success")
		elseif engineHealth >= 300.0 then
			SetVehicleEngineHealth(vehicle, 800.0)
			CTRPFW.Functions.Notify("Engine Health 80% now" , "success")
		elseif engineHealth >= 200.0 then
			SetVehicleEngineHealth(vehicle, 700.0)
			CTRPFW.Functions.Notify("Engine Health 70% now" , "success")
		elseif engineHealth >= 100.0 then
			SetVehicleEngineHealth(vehicle, 600.0)
			CTRPFW.Functions.Notify("Engine Health 60% now" , "success")
		elseif engineHealth >= 0.0 then
			SetVehicleEngineHealth(vehicle, 500.0)
			CTRPFW.Functions.Notify("Engine Health 50% now" , "success")
		end
		SetVehicleTyreFixed(vehicle, 0)
		SetVehicleTyreFixed(vehicle, 1)
		SetVehicleTyreFixed(vehicle, 2)
		...
		TriggerServerEvent('ctrp-vehiclefailure:removeItem', "repairkit")
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		CTRPFW.Functions.Notify("Failed!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if showPro then
            CTRPFW.Functions.DrawText3D(pos.x, pos.y, pos.z, TimeLeft .. '~g~%')
        end
    end
end)
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if brake > 127 then
								...
								--exports['ctrp-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes", exports['ctrp-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes") - 0.01)
							end
...
function DamageRandomComponent()
	...
	--exports['ctrp-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent, exports['ctrp-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent) - randomDamage)
end

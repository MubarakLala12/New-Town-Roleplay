...
--
		if dist <= 0.5 then
			...
			TriggerServerEvent("ctrp-chickenjob:startChicken")
			LapChicken()
			end			
		end
	end
...
if share == true then
	...
	andsplashed = 0
	share = false
	CTRPFW.Functions.Notify("Take Alived Chiken To Process Area ..", "primary")
	TriggerServerEvent("ctrp-chickenjob:getNewChicken")
	end
...
function hewassplashed()
	...
	if chanceofsplashes <= 60 then
			CTRPFW.Functions.Notify("You managed to catch 1 chicken!", "success")
			if Caught1 == 1 then
		...
		CTRPFW.Functions.Notify("The chicken escaped your arms!", "error")
	end
end
------------------DEVELOPED BY VIRUS-----------------------------------------

function packingg(stanowisko)
	local inventory =  CTRPFW.Functions.GetPlayerData()
	...
	CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(result)
		if result then
			...
			CTRPFW.Functions.Progressbar("wash-", "Packing..", 20000, false, true, {
			...
			CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(result4)
				if result4 then
					TriggerServerEvent("ctrp-chickenjob:getpackedChicken",2)
					CTRPFW.Functions.Notify("Keep packing the chicken or go to the vehicle and store it.", "primary")
				else
					print("badhe teej bannre the na xD")
					CTRPFW.Functions.Notify("You have nothing to pack!", "error")
...
function packed(stanowisko)
	...
	LoadDict('anim@heists@box_carry@')
		...
		CTRPFW.Functions.Notify("You stopped packing!", "error")
...
function portioning(position)
	...
	CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(result)
		if result then
			...
			CTRPFW.Functions.Progressbar("Cut-", "Slaughtering..", 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(result1)
				if result1 then	
					CTRPFW.Functions.Notify("Now Pack slaughtered chicken!", "primary")
					TriggerServerEvent("ctrp-chickenjob:getcutChicken", 1)
				else
					print("badhe teej bannre the na xD")
					CTRPFW.Functions.Notify("You dont have any chickens!", "error")
					FreezeEntityPosition(GetPlayerPed(-1),false)
				end
			end, 'alivechicken')
			FreezeEntityPosition(GetPlayerPed(-1),false)
			...
			elseif position == 3 then
				SetEntityHeading(GetPlayerPed(-1), 226.07)
...
function packedsell()
	...
	CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(result)
		if result then
			...
			CTRPFW.Functions.Progressbar("Cut-", "Selling..", 10000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
				...
				CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(result1)
					if result1 then	
						TriggerServerEvent("ctrp-chickenjob:sell",3)
					else
						print("badhe teej bannre the na xD")
						CTRPFW.Functions.Notify("You have nothing to sell!", "error")
						FreezeEntityPosition(GetPlayerPed(-1),false)
					end
				end, 'packagedchicken')

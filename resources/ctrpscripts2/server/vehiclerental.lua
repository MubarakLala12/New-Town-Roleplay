...
RegisterServerEvent('ctrp-vehiclerental:server:SetVehicleRented')
AddEventHandler('ctrp-vehiclerental:server:SetVehicleRented', function(plate, bool, vehicleData)
    local src = source
    ...
    if bool then
        if ply.PlayerData.money.cash >= vehicleData.price then
            ...
            TriggerClientEvent('ctrp-vehiclerental:server:SpawnRentedVehicle', src, plate, vehicleData)

...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        ---- BUITEN
        -- if PlayerJob.name == "recycle1" or PlayerJob.name == "recycle2" or PlayerJob.name == "recycle3" then
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z, true) < 1.3 then
                DrawText3D(Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z + 1, "~g~E~w~ - Enter")
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z, true) < 15 and not IsPedInAnyVehicle(GetPlayerPed(-1), false) and not onDuty then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z, true) < 1.3 then
                DrawText3D(Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z + 1, "~g~E~w~ - Exit")
                if IsControlJustReleased(0, Keys["E"]) then
                    DoScreenFadeOut(500)
                    while not IsScreenFadedOut() do
                        Citizen.Wait(10)
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, 1049.15,-3100.63,-39.95, true) < 15 and not IsPedInAnyVehicle(GetPlayerPed(-1), false) and carryPackage == nil then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, 1049.15,-3100.63,-39.95, true) < 1.3 then
                if onDuty then
                    ...
                    DrawText3D(1049.15,-3100.63,-39.95 + 1, "~g~E~w~ -  To clock in(5000$)")
                end
                if IsControlJustReleased(0, Keys["E"]) then
                    -- if PlayerJob.name == "recycle1" or PlayerJob.name == "recycle2" or PlayerJob.name == "recycle3" then 
                        onDuty = not onDuty
                        if onDuty then
                            CTRPFW.Functions.Notify("You have clocked in")
                            TriggerServerEvent("ctrp-recycle:takemoney")
                        else
                            CTRPFW.Functions.Notify("You have clocked out!", "error")
                        end
                    -- else
                    --     CTRPFW.Functions.Notify("You are not authorised", "error")
                    -- end
                end
            end
        end
        -- if PlayerJob.name == "recycle1" then
        --     inRange = true
        --     if (GetDistanceBetweenCoords(pos, Config.Locations["rstash"].x, Config.Locations["rstash"].y, Config.Locations["rstash"].z, true) < 1.5) then
        --         DrawText3D(Config.Locations["rstash"].x, Config.Locations["rstash"].y, Config.Locations["rstash"].z, "[E] Personal Stash")
        --         if IsControlJustReleased(0, Keys["E"]) then
        --             TriggerEvent("inventory:client:SetCurrentStash", "recyclestash")
        --             TriggerServerEvent("inventory:server:OpenInventory", "stash", "recyclestash", {
        --                 maxweight = 4000000,
        --                 slots = 500,
        --             })
        --         end
                ...
                if carryPackage == nil then
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, packagePos.x,packagePos.y,packagePos.z, true) < 2.3 then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            CTRPFW.Functions.Progressbar("pickup_reycle_package", "Picking up box..", 5000, false, true, {
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, true) < 2.0 then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            DropPackage()
                            ScrapAnim()
                            CTRPFW.Functions.Progressbar("deliver_reycle_package", "Unpacking the package..", 5000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                TriggerServerEvent('ctrp-recycle:server:getItem')
                                GetRandomPackage()
                            end)
                        end
                    else

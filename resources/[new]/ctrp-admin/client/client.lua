...
local menu = MenuV:CreateMenu(false, 'Admin Menu', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test')
local menu2 = MenuV:CreateMenu(false, 'Admin Options', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test1')
local menu4 = MenuV:CreateMenu(false, 'Online Players', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test3')
local menu5 = MenuV:CreateMenu(false, 'Manage Server', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test4')
local menu6 = MenuV:CreateMenu(false, 'Available Weather Options', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test5')
local menu7 = MenuV:CreateMenu(false, 'Dealer List', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test6')
local menu8 = MenuV:CreateMenu(false, 'Ban', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test7')
local menu9 = MenuV:CreateMenu(false, 'Kick', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test8')
local menu10 = MenuV:CreateMenu(false, 'Permissions', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test9')
local menu11 = MenuV:CreateMenu(false, 'Developer Options', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test10')
local menu12 = MenuV:CreateMenu(false, 'Vehicle Options', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test11')
local menu13 = MenuV:CreateMenu(false, 'Vehicle Categories', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test12')
local menu14 = MenuV:CreateMenu(false, 'Vehicle Models', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test13')

RegisterNetEvent('ctrp-admin:client:openMenu')
AddEventHandler('ctrp-admin:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
AddEventHandler('CTRPFW:Client:OnPlayerLoaded', function()
    if devil then
        CTRPFW.Functions.TriggerCallback("ctrp-admin:server:getrank",function(gameode)
            if gameode then
...
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do
        local menu_button14 = menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('ctrp-weathersync:server:setWeather', selection.value)
            CTRPFW.Functions.Notify('Weather Changed To: '..selection.label)
        end})
    end
end)
...
local menu12_button5 = menu12:AddButton({
    ...
    label = 'Get Keys',
    value = 'keys',
    description = 'Get Keys of Vehicle'
})
...
function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        local menu_button10 = menu14:AddButton({
             ...
             select = function(btn)
                 TriggerServerEvent('CTRPFW:CallCommand', "sv", { k })
             end
        })
    end
end
...
menu12_button2:On('Select', function(item)
    TriggerServerEvent('CTRPFW:CallCommand', "fix", {})
end)

menu12_button3:On('Select', function(item)
    TriggerServerEvent('CTRPFW:CallCommand', "admincar", {})
end)
...
menu12_button4:On('Select', function(item)
    ...
    if IsPedInAnyVehicle(ped, false) then
        TriggerServerEvent('vehiclekeys:server:SetVehicleOwner', GetVehicleNumberPlateText(vehicle), 'remove')
    end
    TriggerServerEvent('CTRPFW:CallCommand', "dv", {})
end)
...
names_button:On('change', function()           
    TriggerEvent('ctrp-admin:client:toggleNames')
end)                                           
blips_button:On('change', function()           
    TriggerEvent('ctrp-admin:client:toggleBlips')
end)

-- Dealer List
-- menu_button4:On('Select', function(item)
--     menu7:ClearItems()
--     CTRPFW.Functions.TriggerCallback('test:getdealers', function(dealers)
...
menu_button2:On('select', function(item)
    menu4:ClearItems()
    CTRPFW.Functions.TriggerCallback('test:getplayers', function(players)
...
menu_button13:On("select", function(item, value)
    TriggerServerEvent("ctrp-weathersync:server:setTime", value, value)
    CTRPFW.Functions.Notify("Time changed to " .. value .. " hs 00 min")

end)

function OpenDealerMenu(dealer)
    local EditDealer = MenuV:CreateMenu(false, 'Edit Dealer ' .. dealer["name"], 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv')
    ...
    for k, v in ipairs(elements) do
        local menu_button10 = EditDealer:AddButton({
            ...
            select = function(btn)
                ...
                if values == "goto" then
                    TriggerServerEvent('CTRPFW:CallCommand', "dealergoto", { dealer["name"] })
                elseif values == "remove" then
                    TriggerServerEvent('CTRPFW:CallCommand', "deletedealer", { dealer["name"] })
                    EditDealer:Close()
                    menu7:Close()
                end
            end
...
function OpenPlayerMenus(player)
    ...
    local Players = MenuV:CreateMenu(false, player.cid .. ' Options', 'centerright', 13, 124, 2, 'size-125', 'none', 'menuv') -- Players Sub Menu
    Players:ClearItems()
    MenuV:OpenMenu(Players)
    local professor = nil
    CTRPFW.Functions.TriggerCallback("ctrp-admin:server:getrank",function(ayush)
        if ayush then
            professor = {
                [1] = {
                    icon = '💀',
    ...
    if ayush2 then
        ...
        for k, v in ipairs(professor) do
            local menu_button10 = Players:AddButton({
                ...
                select = function(btn)
                    ...
                    if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                        TriggerServerEvent('ctrp-admin:server:'..values, player)
                    ...
                    elseif values == "chatko" then
                        TriggerServerEvent('ctrp-admin:server:chatko', player)
                    end
                end
            })
        end
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('ctrp-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                CTRPFW.Functions.Notify('You must give a Reason and set a Length for the ban!', 'error')
            end
        end
    })
end
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('ctrp-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                CTRPFW.Functions.Notify('You must give a reason!', 'error')
            end
        end
    })
end
...
function OpenPermsMenu(permsply)
    CTRPFW.Functions.TriggerCallback('ctrp-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('ctrp-admin:server:setPermissions', permsply.id, selectedgroup)
			CTRPFW.Functions.Notify('Authority group changed!', 'success')
                        selectedgroup = 'Unknown'
                    else
                        CTRPFW.Functions.Notify('Choose a group!', 'error')
                    end
                end
            })
        else

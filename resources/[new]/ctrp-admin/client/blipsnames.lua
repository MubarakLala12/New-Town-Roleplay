...
RegisterNetEvent('ctrp-admin:client:toggleBlips')
AddEventHandler('ctrp-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        CTRPFW.Functions.Notify("Blips activated", "success")
    else
        ShowBlips = false
        CTRPFW.Functions.Notify("Blips deactivated", "error")
    end
end)

RegisterNetEvent('ctrp-admin:client:toggleNames')
AddEventHandler('ctrp-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        CTRPFW.Functions.Notify("Names activated", "success")
    else
        ShowNames = false
        CTRPFW.Functions.Notify("Names deactivated", "error")
    end
end)

RegisterNetEvent('ctrp-admin:client:Show')
AddEventHandler('ctrp-admin:client:Show', function(players)
        ...
        if ShowBlips then
            if not DoesBlipExist(blip) then -- Blip doesn't exist, make it appear
                ...
                elseif veh ~= 0 then -- Check if ped is in vehicle
                    ...
                    elseif blipSprite ~= 326 then -- default blip
...
Citizen.CreateThread(function()
    while true do
        Wait(3000)
        if ShowBlips then
            TriggerServerEvent('ctrp-admin:server:GetPlayersForBlips')
        end
    end
end)

...
AddEventHandler('CTRPFW:Client:OnPlayerLoaded', function()
    disable = false
    TriggerServerEvent('ctrp-weathersync:server:RequestStateSync')
    TriggerServerEvent('ctrp-weathersync:server:RequestCommands')
end)

RegisterNetEvent('ctrp-weathersync:client:EnableSync')
AddEventHandler('ctrp-weathersync:client:EnableSync', function()
    disable = false
    TriggerServerEvent('ctrp-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('ctrp-weathersync:client:DisableSync')
AddEventHandler('ctrp-weathersync:client:DisableSync', function()
	disable = true
...
RegisterNetEvent('ctrp-weathersync:client:SyncWeather')
AddEventHandler('ctrp-weathersync:client:SyncWeather', function(NewWeather, newblackout)
    CurrentWeather = NewWeather
    blackout = newblackout
end)

RegisterNetEvent('ctrp-weathersync:client:RequestCommands')
AddEventHandler('ctrp-weathersync:client:RequestCommands', function(isAllowed)
    if isAllowed then
...
RegisterNetEvent('ctrp-weathersync:client:SyncTime')
AddEventHandler('ctrp-weathersync:client:SyncTime', function(base, offset, freeze)
    freezeTime = freeze
    timeOffset = offset
    baseTime = base
end)

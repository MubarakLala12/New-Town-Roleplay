...
RegisterNetEvent("virus:selector:show", function(data)
    if data then
        BuildSelector(data);
    else
        TriggerServerEvent("virus:selector:show")
    end
end)
...
function BuildLocations(data)
    ...
    if data then
        ...
        for i=1, #Settings.Locations do
            ...
            if (location.name == "prison" and data.jail) or (not location.job or location.job == data.job) and not data.jail then
...
--[[RegisterCommand('test1234', function(source)

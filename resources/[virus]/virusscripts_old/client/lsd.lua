	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.MushroomProcessing.coords, true) < 0.5 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				CTRPFW.Functions.TriggerCallback('mushroom:ingredient', function(HasItem, type)
					if HasItem then
						ProcessMushroom()
					else
...
function ProcessMushroom()
	...
	CTRPFW.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
		...
		CTRPFW.Functions.TriggerCallback('mushroom:ingredient', function(HasItem, type)
			if HasItem then	
				TriggerServerEvent('virus-lsdmushroom:processMushroom')
			else
		...
		while timeLeft > 0 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.MushroomProcessing.coords, false) > 4 then
				TriggerServerEvent('virus-lsdmushroom:cancelProcessing')
				break
			end
		end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				CTRPFW.Functions.Progressbar("search_register", "Picking up Mushroom..", 10000, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
					...
					TriggerServerEvent('virus-lsdmushroom:pickedMushroom')
...
function SpawnmushroomGrowth()
    ...
    while b < random do
        ...
        local E = CreateObject(3267161942, D.x, D.y, D.z, false, false, true)

...
--[[ NUI Callbacks ]]--
    ...
    RegisterNUICallback("spawnk9", function(data)
        TriggerEvent("virus-pethouse:ToggleK9", data.model)
    end)

    RegisterNUICallback("vehicletoggle", function(data)
        if spawned_ped ~= nil then
            TriggerServerEvent("virus-pethouse:RequestVehicleToggle")
        end
    end)

    RegisterNUICallback("vehiclesearch", function(data)
        if spawned_ped ~= nil then
            TriggerServerEvent("virus-pethouse:RequestItems")
        end
    end)
...
--[[ Main Event Handlers ]]--
    ...
    RegisterNetEvent("virus-pethouse:UpdateLanguage")
    AddEventHandler("virus-pethouse:UpdateLanguage", function(commands)
        language = commands
        Citizen.Trace(tostring(json.encode(language)))
    end)

    -- Opens K9 Menu
    RegisterNetEvent("virus-pethouse:OpenMenu")
    AddEventHandler("virus-pethouse:OpenMenu", function(pedRestriction, pedList)
    ...
    RegisterNetEvent("virus-pethouse:IdentifierRestricted")
    AddEventHandler("virus-pethouse:IdentifierRestricted", function()
        Notification(tostring("~r~You do not match any identifiers in the whitelist."))
    end)


    RegisterNetEvent("virus-pethouse:sit")
    AddEventHandler("virus-pethouse:sit", function()
        if spawned_ped ~= nil then
            PlayAnimation(animations['Normal'].sit.dict, animations['Normal'].sit.anim)
        else
            CTRPFW.Functions.Notify("You don't have Pet out!", "error")
        end
    end)

    RegisterNetEvent("virus-pethouse:laydown")
    AddEventHandler("virus-pethouse:laydown", function()
        if spawned_ped ~= nil then
            PlayAnimation(animations['Normal'].laydown.dict, animations['Normal'].laydown.anim)
        else
            CTRPFW.Functions.Notify("You don't have Pet out!", "error")
    ...
    RegisterNetEvent("virus-pethouse:ToggleK9")
    AddEventHandler("virus-pethouse:ToggleK9", function(model)
        if spawned_ped == nil then
            ...
            AddTextComponentString(tostring("virus-pethouse: ".. k9_name))
    ...
    RegisterNetEvent("virus-pethouse:ToggleFollow")
    AddEventHandler("virus-pethouse:ToggleFollow", function()
        if spawned_ped ~= nil then
    ...
    RegisterNetEvent("virus-pethouse:ToggleVehicle")
    AddEventHandler("virus-pethouse:ToggleVehicle", function(isRestricted, vehList)
    ...
    RegisterNetEvent("virus-pethouse:ToggleAttack")
    AddEventHandler("virus-pethouse:ToggleAttack", function(target)
    ...
    RegisterNetEvent("virus-pethouse:SearchVehicle")
    AddEventHandler("virus-pethouse:SearchVehicle", function(items, openDoors)
        local vehicle = GetVehicleAheadOfPlayer()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if IsControlPressed(1, 19) and IsControlJustPressed(1, 213) then
                TriggerServerEvent("virus-pethouse:RequestOpenMenu")
            end

            -- Trigger Attack
            if IsControlJustPressed(1, 289) and IsPlayerFreeAiming(PlayerId()) then
                local bool, target = GetEntityPlayerIsFreeAimingAt(PlayerId())

                if bool then
                    if IsEntityAPed(target) then
                        --TriggerEvent("virus-pethouse:ToggleAttack", target)
                        CTRPFW.Functions.Notify(k9_name.." No Bite !" , "error", 2500)
                    end
                end
            end

            -- Trigger Follow
            if IsControlJustPressed(1, 289) and not IsPlayerFreeAiming(PlayerId()) then
                TriggerEvent("virus-pethouse:ToggleFollow")
            end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if just_started then
                ...
                just_started = false
                TriggerServerEvent("virus-pethouse:SendLanguage")
            end

            -- Deletes K9 when you die
            if spawned_ped ~= nil and IsEntityDead(GetLocalPed()) then
                TriggerEvent("virus-pethouse:ToggleK9")
            end
        end
    end)

...
VirusFWAdmin = {}
VirusFWAdmin.Functions = {}
in_noclip_mode = false

VirusFWAdmin.Functions.DrawText3D = function(x, y, z, text, lines)
...
AddEventHandler('CTRPFW:Client:OnPlayerLoaded', function()
    TriggerServerEvent("virus-god:server:loadPermissions")
end)
...
RegisterNetEvent('virus-god:client:openMenu')
AddEventHandler('virus-god:client:openMenu', function(group, dealers)
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('admin', 'CTRP GOD MENU')
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('adminOpt') then
            ...
            if WarMenu.Button("Refresh Skin") then
                WarMenu.CloseMenu()
                TriggerEvent('CTRPFW:Client:Refreshskin')
            end

            if WarMenu.Button("Clothing Menu") then
                WarMenu.CloseMenu()
                TriggerEvent('ctrp-clothing:client:openClothingMenu')
            end
            ...
            if WarMenu.Button("Barber Menu") then
                WarMenu.CloseMenu()
                TriggerEvent('ctrp-clothing:client:openBarberMenu')
            end

            if WarMenu.Button("Tattoos Menu") then
                WarMenu.CloseMenu()
                
                TriggerEvent('virus-tattoo:client:tattooppen')
            end

            if WarMenu.Button('Revive') then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                TriggerServerEvent('virus-god:server:revivePlayer', targetId)
            end
            
            if WarMenu.Button('Uncuff') then
                TriggerServerEvent('police:server:unnCuffPlayer')
            end

            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                TriggerServerEvent("virus-god:server:togglePlayerNoclip", targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('managementOptions') then
            ...
            if WarMenu.ComboBox('Weapon', Weapons, currentWeaponIndex, selectedWeaponIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent("virus-god:server:spawnWeapon", WeaponsHashs[currentWeaponIndex])
            end

            if WarMenu.MenuButton('Max Ammo', 'managementOptions') then
                TriggerEvent('ctrp-weapons:client:SetWeaponAmmoManual', 'current', 250)
            end
        ...
        elseif WarMenu.IsMenuOpened('vehOptions') then
            ...
            if WarMenu.MenuButton('Set Owned', 'vehOptions') then
				...
				local props = CTRPFW.Functions.GetVehicleProperties(vgehicle)
            ...
            if WarMenu.MenuButton('Fill Fuel', 'vehOptions') then
                local vehicle = (GetVehiclePedIsIn(PlayerPedId()))
                exports['ctrp-hud']:SetFuel(vehicle, 99)
            end
            ...
            if WarMenu.MenuButton('Delete Vehicle', 'vehOptions') then
                CTRPFW.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            end
        ...
        elseif WarMenu.IsMenuOpened('serverMan') then
            ...
            if WarMenu.ComboBox('Server time', times, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent("ctrp-weathersync:server:setTime", time.hour, time.minute)
            end
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            if WarMenu.MenuButton('Crash', currentPlayer) then
                TriggerServerEvent('virus-god:server:crash', GetPlayerServerId(currentPlayer))
            end
            
            if WarMenu.MenuButton('Kill', currentPlayer) then
                TriggerServerEvent("virus-god:server:killPlayer", GetPlayerServerId(currentPlayer))
            end
            if WarMenu.MenuButton('Revive', currentPlayer) then
                TriggerServerEvent('virus-god:server:revivePlayer', GetPlayerServerId(currentPlayer))
            end
            
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                TriggerServerEvent("virus-god:server:togglePlayerNoclip", GetPlayerServerId(currentPlayer))
            end
            
            if WarMenu.CheckBox("Freeze", isFreeze, function(checked) isFreeze = checked end) then
                TriggerServerEvent("virus-god:server:Freeze", GetPlayerServerId(currentPlayer), isFreeze)
            end
            ...
            if WarMenu.MenuButton("Give Clothing Menu", currentPlayer) then
                TriggerServerEvent('virus-god:server:OpenSkinMenu', GetPlayerServerId(currentPlayer))
            end

            if WarMenu.MenuButton("Give Barber Menu", currentPlayer) then
                TriggerServerEvent('virus-god:server:OpenBarberMenu', GetPlayerServerId(currentPlayer))
            end

            if WarMenu.MenuButton("Give Tattoos Menu", currentPlayer) then
                TriggerServerEvent('virus-god:server:OpenTattooMenu', GetPlayerServerId(currentPlayer))
            end
        ...
        elseif WarMenu.IsMenuOpened('teleportOptions') then
            if WarMenu.MenuButton('Goto', currentPlayer) then
                if in_noclip_mode then
                    turnNoClipOff()
                    TriggerServerEvent('virus-god:server:gotoTp', GetPlayerServerId(currentPlayer), GetPlayerServerId(PlayerId()))
            ...
            if WarMenu.MenuButton('Bring', currentPlayer) then
                ...
                TriggerServerEvent('virus-god:server:bringTp', GetPlayerServerId(currentPlayer), plyCoords)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent('virus-god:server:setPermissions', target, group)

                CTRPFW.Functions.Notify('You have ' .. GetPlayerName(currentPlayer) .. '\'s group has changed to '..group.label)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('adminOptions') then
            if WarMenu.ComboBox('Ban Length', bans, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                if reason ~= nil and reason ~= "" and time ~= 0 then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("virus-god:server:banPlayer", target, time, reason)
                end
            end
            if WarMenu.MenuButton('Kick', currentPlayer) then
                ...
                if reason ~= nil and reason ~= "" then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("virus-god:server:kickPlayer", target, reason)
                end
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                    TriggerServerEvent('ctrp-weathersync:server:setWeather', AvailableWeatherTypes[k].weather)
                    CTRPFW.Functions.Notify('Again has changed to: '..AvailableWeatherTypes[k].label)
                end
            end
        ...
        elseif WarMenu.IsMenuOpened('createDealer') then
            if WarMenu.ComboBox('Min. Time', times, currentMinTimeIndex, selectedMinTimeIndex, function(currentIndex, selectedIndex)
                ...
                CTRPFW.Functions.Notify('Time confirmed!', 'success')
            end
    ...
    while true do
        ...
        if showNames then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(GetPlayerPed(-1)), 5.0)) do
                ...
                VirusFWAdmin.Functions.DrawText3D(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z + 1.0, '['..PlayerId..'] '..PlayerName)
            end
        else
            Citizen.Wait(1000)
        end
	...
	while true do
        ...
        if deleteLazer then
            ...
            if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                ...
                VirusFWAdmin.Functions.DrawText3D(entityCoord.x, entityCoord.y, entityCoord.z, "Object: " .. entity .. " Model: " .. GetEntityModel(entity).. " \nPress [~g~E~s~] to delete this object.", 2)
...
RegisterNetEvent('virus-god:client:bringTp')
AddEventHandler('virus-god:client:bringTp', function(coords)
    local ped = GetPlayerPed(-1)

    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('virus-god:client:gotoTp')
AddEventHandler('virus-god:client:gotoTp', function(targetId)
    local ped = GetPlayerPed(-1)
    local coords = GetEntityCoords(ped)
    TriggerServerEvent('virus-god:server:gotoTpstage2', targetId, coords)
end)

RegisterNetEvent('virus-god:client:gotoTp2')
AddEventHandler('virus-god:client:gotoTp2', function(coords)
    print(coords)
    local ped = GetPlayerPed(-1)
    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('virus-god:client:Freeze')
AddEventHandler('virus-god:client:Freeze', function(toggle)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('virus-god:client:SendReport')
AddEventHandler('virus-god:client:SendReport', function(name, src, msg)
    TriggerServerEvent('virus-god:server:SendReport', name, src, msg)
end)

RegisterNetEvent('virus-god:client:SendGodChat')
AddEventHandler('virus-god:client:SendGodChat', function(name, msg)
    TriggerServerEvent('virus-god:server:GodChatMessage', name, msg)
end)

RegisterNetEvent('virus-god:client:SaveCar')
AddEventHandler('virus-god:client:SaveCar', function()
    local ped = GetPlayerPed(-1)
    local veh = GetVehiclePedIsIn(ped)
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if CTRPFW.Shared.VehicleModels[hash] ~= nil and next(CTRPFW.Shared.VehicleModels[hash]) ~= nil then
            TriggerServerEvent('virus-god:server:SaveCar', props, CTRPFW.Shared.VehicleModels[hash], GetHashKey(veh), plate)
        else
            CTRPFW.Functions.Notify('You cannot put this vehicle in your garage..', 'error')
        end
    else
...
RegisterNetEvent('virus-god:client:SetModel')
AddEventHandler('virus-god:client:SetModel', function(skin)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('virus-god:client:SetSpeed')
AddEventHandler('virus-god:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('virus-god:client:GiveNuiFocus')
AddEventHandler('virus-god:client:GiveNuiFocus', function(focus, mouse)
    SetNuiFocus(focus, mouse)
end)

RegisterNetEvent('virus-god:client:EnableKeys')
AddEventHandler('virus-god:client:EnableKeys', function()
    print('[virus-god] Enabled Keys.')
    EnableAllControlActions(0)
    SetNuiFocus(true, true)
end)

RegisterNetEvent('virus-god:client:crash')
AddEventHandler('virus-god:client:crash', function()
    while true do end
end)
...
-- ADDON
--[[
    RegisterNetEvent('ctrp-clothing:client:openOutfitMenu')
AddEventHandler('ctrp-clothing:client:openOutfitMenu', function()
    CTRPFW.Functions.TriggerCallback('ctrp-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('ctrp-clothing:client:openBarberMenu')
AddEventHandler('ctrp-clothing:client:openBarberMenu', function()
    customCamLocation = nil
    openMenu({
        
        {menu = "clothing", label = "Hair", selected = true, outfits = result},
    })
end)

RegisterNetEvent('ctrp-clothing:client:openClothingMenu')
AddEventHandler('ctrp-clothing:client:openClothingMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNetEvent('virus-tattoo:client:tattooppen')
AddEventHandler('virus-tattoo:client:tattooppen', function()
    OpenTattooShop()
end)

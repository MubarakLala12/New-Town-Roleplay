...
Citizen.CreateThread(function()
	while true do
		...
		if not IsMenuOpen() then
			CTRPFW.Functions.TriggerCallback('virus-tattoos:GetPlayerTattoos', function(tattooList)
				if tattooList then
...
function GetNaked()
	...
	if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
		...
		TriggerEvent('ctrp-clothing:client:loadOutfit', outfitMan)
	elseif GetEntityModel(PlayerPedId()) == `mp_f_freemode_01` then
		...
		TriggerEvent('ctrp-clothing:client:loadOutfit', outfitFemale)
	else
		CTRPFW.Functions.TriggerCallback('virus-tattoos:GetPlayerTattoos', function(tattooList)
			if tattooList then
				ClearPedDecorations(PlayerPedId())
...
function BuyTattoo(collection, name, label, price)
	CTRPFW.Functions.TriggerCallback('virus-tattoos:PurchaseTattoo', function(success)
		if success then
...
function RemoveTattoo(name, label)
	...
	TriggerServerEvent("virus-tattoos:RemoveTattoo", currentTattoos)
	CTRPFW.Functions.Notify("You have tattoo "..GetLabelText(label).. " deleted.", "error", 2500)
end
...
function CreateScale(sType)
	...
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Camera Weergave", 21, "Color Thickness", {90, 89}, "Buy / Remove Tattoo", 191)
		scaleString = sType
	end
end
		...
		if not IsMenuOpen() then
			for k, v in pairs(Config.Shops) do
				...
				if dist < 30 then
					...
					CanSleep = false
					if dist < 2.5 then
						DrawText3Ds(Config.Shops[k].x, Config.Shops[k].y, Config.Shops[k].z + 0.25, '~g~E~w~ - To get tattoo')

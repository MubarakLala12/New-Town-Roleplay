...
doAction = function(veh)
	...
	CTRPFW.Functions.TriggerCallback('CTRPFW:HasItem', function(result)
		if result then
			...
			--CTRPFW.Functions.Notify('Started Repairing', 'Success')
			CTRPFW.Functions.Notify('POSITIONING THE JACK', 'Success')
			...
			CTRPFW.Functions.Notify('ADJUSTING THE VEHICLE', 'Success')
			...
			CTRPFW.Functions.Notify('LEAVING THE CRICK', 'Success')
			TaskPlayAnimAdvanced(ped, dict, 'car_bomb_mechanic', coords, 0.0, 0.0, headin, 1.0, 0.5, 1250, 1, 0.0, 1, 1)
			Citizen.Wait(1250)
			SetEntityCoordsNoOffset(veh, vehpos.x, vehpos.y, vehpos.z + 0.4, true, true, true)
			TaskPlayAnimAdvanced(ped, dict, 'car_bomb_mechanic', coords, 0.0, 0.0, headin, 1.0, 0.5, 1000, 1, 0.25, 1, 1)
...
RegisterNetEvent("virus-repair:client:triggerMenu")
AddEventHandler("virus-repair:client:triggerMenu", function()

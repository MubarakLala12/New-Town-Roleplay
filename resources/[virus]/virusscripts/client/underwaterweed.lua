	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones3.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				CTRPFW.Functions.TriggerCallback('virus-underwaterweed:ingredient', function(HasItem, type)
					if HasItem then
						ProcessWeed()
					else
...
function ProcessWeed()
	...
	CTRPFW.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
		...
		CTRPFW.Functions.TriggerCallback('virus-underwaterweed:ingredient', function(HasItem, type)
			if HasItem then	
				TriggerServerEvent('virus-underwaterweed:processWeed')
			else
		...
		while timeLeft > 0 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones3.WeedProcessing.coords, false) > 4 then
				TriggerServerEvent('virus-underwaterweed:cancelProcessing')
				break
			end
		end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				CTRPFW.Functions.Progressbar("search_register", "Picking up..", 5000, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
					...
					TriggerServerEvent('virus-underwaterweed:pickedUpWeed')
					TriggerServerEvent('virus-underwaterweed:weed')
					ClearPedTasks(PlayerPedId())

...
RegisterNetEvent('virus-banking:client:newCardSuccess')
AddEventHandler('virus-banking:client:newCardSuccess', function(cardno, ctype)
    SendNUIMessage({
        status = "updateCard",
        number = cardno,
        cardtype = ctype
...
RegisterNUICallback("createSavingsAccount", function(data, cb)
    TriggerServerEvent('virus-banking:createSavingsAccount')
...
RegisterNUICallback("doDeposit", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        ...
        TriggerServerEvent('virus-banking:doQuickDeposit', data.amount)
        openAccountScreen()
    end
end)
...
RegisterNUICallback("doWithdraw", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        ...
        TriggerServerEvent('virus-banking:doQuickWithdraw', data.amount, true)
        openAccountScreen()
    end
end)
...
RegisterNUICallback("doATMWithdraw", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        ...
        TriggerServerEvent('virus-banking:doQuickWithdraw', data.amount, false)
        openAccountScreen()
    end
end)
...
RegisterNUICallback("doTransfer", function(data, cb)
    if data ~= nil then
        TriggerServerEvent('virus-banking:initiateTransfer', data)
    end
end)

RegisterNUICallback("createDebitCard", function(data, cb)
    if data.pin ~= nil then
        TriggerServerEvent('virus-banking:createBankCard', data.pin)
    end
end)

RegisterNUICallback("lockCard", function(data, cb)
    TriggerServerEvent('virus-banking:toggleCard', true)
end)

RegisterNUICallback("unLockCard", function(data, cb)
    TriggerServerEvent('virus-banking:toggleCard', false)
end)

RegisterNUICallback("updatePin", function(data, cb)
    if data.pin ~= nil then 
        TriggerServerEvent('virus-banking:updatePin', data.pin)

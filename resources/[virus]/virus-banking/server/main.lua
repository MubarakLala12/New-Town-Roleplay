...
Citizen.CreateThread(function()
    ...
    CTRPFW.Functions.ExecuteSql(true, "SELECT * FROM `banks`", function(bankssql)
    ...
    CTRPFW.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_type` = 'Buisness'", function(accts)
        buis = #accts
        if accts[1] ~= nil then
    ...
    CTRPFW.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_type` = 'Savings'", function(savings)
        sav = #savings
    ...
    CTRPFW.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_type` = 'Gang'", function(gangs)
        gang = #gangs
        if gangs[1] ~= nil then
...
RegisterServerEvent('virus-banking:server:modifyBank')
AddEventHandler('virus-banking:server:modifyBank', function(bank, k, v)
    if banks[tonumber(bank)] then
        banks[tonumber(bank)][k] = v
        TriggerClientEvent('virus-banking:client:syncBanks', -1, banks)
    end
end)

exports('modifyBank', function(bank, k, v)
    TriggerEvent('virus-banking:server:modifyBank', bank, k, v)
end)
...
CTRPFW.Functions.CreateCallback('virus-banking:server:requestBanks', function(source, cb)
    repeat Wait(0) until banks ~= nil
    cb(banks)
end)

CTRPFW.Functions.CreateCallback('virus-banking:server:checkMoneyBagCount', function(source, cb)
    local src = source
...
function checkAccountExists(acct, sc)
    ...
    CTRPFW.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_number` = '" .. acct .. "' AND `sort_code` = '" .. sc .. "'", function(exists)
...
RegisterServerEvent('virus-banking:createNewCard')
AddEventHandler('virus-banking:createNewCard', function()
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
    ...
    TriggerEvent('ctrp-logs:server:createLog', 'banking', 'Banking', "Created new card **[" .. xPlayer.PlayerData.citizenid .. "]**", src)
end)

RegisterServerEvent('virus-base:itemUsed')
AddEventHandler('virus-base:itemUsed', function(_src, data)
    if data.item == "moneybag" then
        TriggerClientEvent('virus-banking:client:usedMoneyBag', _src, data)
    end
end)

RegisterServerEvent('virus-banking:server:unpackMoneyBag')
AddEventHandler('virus-banking:server:unpackMoneyBag', function(item)
    local _src = source
    if item ~= nil then
        local xPlayer = CTRPFW.Functions.GetPlayer(_src)
...
RegisterServerEvent('virus-banking:initiateTransfer')
AddEventHandler('virus-banking:initiateTransfer', function(data)
    --[[
    local _src = source
    local _startChar = CTRPFW.Functions.GetPlayer(_src)
    ...
    if (checkAccount) then 
        ...
        if receiptName ~= false or receiptName ~= nil then 
            local userOnline = exports.virus-base:checkOnline(cid)
            
            if userOnline ~= false then
                -- User is online so we can do a straght transfer 
                local _targetUser = exports.virus-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('virus-banking:openBankScreen', _src)
                    TriggerClientEvent('virus-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('virus-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
CTRPFW.Functions.CreateCallback('virus-banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
...
RegisterServerEvent('virus-banking:createBankCard')
AddEventHandler('virus-banking:createBankCard', function(pin)
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
    local cid = xPlayer.PlayerData.citizenid
    ...
    TriggerClientEvent('virus-banking:openBankScreen', src)
    TriggerClientEvent('CTRPFW:Notify', src, 'You have successfully ordered a Debit Card.', 'success')
    
    TriggerEvent('ctrp-logs:server:createLog', 'banking', 'Banking', 'Successfully ordered a Debit Card', src)
end)

RegisterServerEvent('virus-banking:doQuickDeposit')
AddEventHandler('virus-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('virus-banking:openBankScreen', _src)
            TriggerClientEvent('virus-banking:successAlert', _src, 'You made a cash deposit of $'..amount..' successfully.')
            TriggerEvent('ctrp-logs:server:createLog', 'banking', 'Banking', 'Made a cash deposit of $'..amount..' successfully.', src)
        end
    end
end)

RegisterServerEvent('virus-banking:toggleCard')
AddEventHandler('virus-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
...
RegisterServerEvent('virus-banking:doQuickWithdraw')
AddEventHandler('virus-banking:doQuickWithdraw', function(amount, branch)
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then 
            TriggerClientEvent('virus-banking:openBankScreen', _src)
            TriggerClientEvent('virus-banking:successAlert', _src, 'You made a cash withdrawal of $'..amount..' successfully.')
            TriggerEvent('ctrp-logs:server:createLog', 'banking', 'Banking', 'Made a cash withdrawal of $'..amount..' successfully.', src)
        end
    end
end)

RegisterServerEvent('virus-banking:updatePin')
AddEventHandler('virus-banking:updatePin', function(pin)
    if pin ~= nil then 
        local src = source
        local xPlayer = CTRPFW.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        TriggerClientEvent('virus-banking:openBankScreen', src)
        TriggerClientEvent('virus-banking:successAlert', src, 'You have successfully updated your Debit card pin.')
    end
end)

RegisterServerEvent('virus-banking:savingsDeposit')
AddEventHandler('virus-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('virus-banking:openBankScreen', src)
        TriggerClientEvent('virus-banking:successAlert', src, 'You made a savings deposit of $'..tostring(amount)..' successfully.')
        TriggerEvent('ctrp-logs:server:createLog', 'banking', 'Banking', 'made a savings deposit of $'..tostring(amount)..' successfully..', src)
    end
end)

RegisterServerEvent('virus-banking:savingsWithdraw')
AddEventHandler('virus-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('virus-banking:openBankScreen', src)
        TriggerClientEvent('virus-banking:successAlert', src, 'You made a savings withdrawal of $'..tostring(amount)..' successfully.')
        TriggerEvent('ctrp-logs:server:createLog', 'banking', 'Banking', 'Made a savings withdrawal of $'..tostring(amount)..' successfully.', src)
    end
end)

RegisterServerEvent('virus-banking:createSavingsAccount')
AddEventHandler('virus-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = CTRPFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('virus-banking:openBankScreen', src)
    TriggerClientEvent('virus-banking:successAlert', src, 'You have successfully opened a savings account.')
    TriggerEvent('ctrp-logs:server:createLog', 'banking', 'Banking', "Created new saving account", src)
end)

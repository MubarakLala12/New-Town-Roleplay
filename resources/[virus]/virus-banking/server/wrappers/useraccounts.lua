function generateCurrent(cid)
    ...
    CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `bank_cards` WHERE `citizenid` = '" .. self.cid .."'", function(getCurrentAccount)
    ...
    CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `bank_statements` WHERE `account` = 'Current' AND `citizenid` = '" .. self.cid .. "' ORDER BY `record_id` DESC LIMIT 30", function(bankStatement)
    ...
    rTable.generateNewCard = function(pin, scc)
        ...
        if not self.cardActive then
            ...
            CTRPFW.Functions.ExecuteSql(false, "UPDATE `bank_cards` SET `cardNumber` = '" .. cardNumber .. "', `cardPin` = '" .. pinSet .. "', `cardDecrypted` = 'false', `cardActive` = 1, `cardLocked` = 0, `cardType` = '" .. friendlyName .. "' WHERE `citizenid` = '" ..self.cid .. "' AND `record_id` = '" .. self.aid .. "'", function(rowsChanged)
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('virus-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = CTRPFW.Functions.GetPlayer(self.source)
...
RegisterServerEvent('virus-banking:server:registerCurrentAccount')
AddEventHandler('virus-banking:server:registerCurrentAccount', function(cid)
...
function generateSavings(cid)
    ...
    CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `bank_accounts` WHERE `citizenid` = '" .. self.cid .."' AND `account_type` = 'Savings'", function(getSavingsAccount)
        if getSavingsAccount[1] ~= nil then
            self.aid = getSavingsAccount[1].record_id
            self.balance = getSavingsAccount[1].amount
        end
        
        CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `bank_statements` WHERE `account` = 'Savings' AND `citizenid` = '" .. self.cid .. "' ORDER BY `record_id` DESC LIMIT 30", function(stats)
    ...
    self.saveAccount = function()
        local success = CTRPFW.Functions.ExecuteSql(false, "UPDATE `bank_accounts` SET `amount` = '" .. self.balance .. "' WHERE `citizenid` = '" .. self.cid .. "' AND `record_id` = '" .. self.aid .. "'")
        if success == 1 then
            return true
        else
            return false
    ...
    rTable.AddMoney = function(amt, text)
        if type(amt) == "number" and text then
            ...
            local time = os.date("%Y-%m-%d %H:%M:%S")
            CTRPFW.Functions.ExecuteSql(false, "INSERT INTO `bank_statements` (`citizenid`,  `account`, `deposited`, `withdraw`, `balance`, `date`, `type`) VALUES ('" .. self.cid .. "', 'Saving','" .. amt .. "', '', '" .. self.balance .. "', '" .. time .. "', '" .. text .. "')")
...
RegisterServerEvent('virus-banking:server:registerSavingsAccount')
AddEventHandler('virus-banking:server:registerSavingsAccount', function(cid)
...
function createSavingsAccount(cid)
    ...
    local success = false
    CTRPFW.Functions.ExecuteSql(false, "INSERT INTO `bank_accounts` (`citizenid`,`amount`,`account_type`) VALUES ('" .. cid .. "', '0', 'Savings')", function(result)
        savingsAccounts[cid] = generateSavings(cid)
        success = true
        completed = true
    end)

...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            isDoingAction = true
            workingVirus = true
            TriggerEvent('inventory:client:notopenplease',isDoingAction)
            wasCancelled = false
            isAnim = false
            isProp = false
            ...
            CTRPFW.Functions.Notify("You are already doing something!", "error")
        end
    else
        CTRPFW.Functions.Notify("Cant do that action!", "error")
    end
end
...
function Cancel()
    isDoingAction = false
    TriggerEvent('inventory:client:openplease',isDoingAction)
    wasCancelled = true
...
RegisterNetEvent("progressbar:client:cancel2")
AddEventHandler("progressbar:client:cancel2", function()
	Cancel2()
end)

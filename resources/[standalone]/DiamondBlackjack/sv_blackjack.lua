...
--[[function tryTakeChips(source,amount)
    ...
    if Player.Functions.RemoveItem("casinochips", amount) then
        ...
        TriggerClientEvent('CTRPFW:Notify', source, "You don't enough casinochips..")
    end
...
function tryTakeChips(source,amount)
    ...
    local Player = CTRPFW.Functions.GetPlayer(source)
    local amount = amount
    local casinochips = Player.Functions.GetItemByName("casinochips")

    if casinochips.amount >= amount then
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent('CTRPFW:Notify', source, "Error, can't sit you down." , "error")
...
AddEventHandler("Blackjack:setBlackjackBet",function(gameId,betAmount,chairId)
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if tryTakeChips(source,betAmount) then
                        ...
                        TriggerClientEvent('CTRPFW:Notify', source, "Bet placed: " .. tostring(betAmount) .. " chips." , "success")
                    else 
                        --TriggerClientEvent("blackjack:notify",source,"~r~Not enough chips!")
                        TriggerClientEvent('CTRPFW:Notify', source, "Not enough chips!." , "error")
                    end
                end
            end
        end
        ...
        TriggerClientEvent('CTRPFW:Notify', source, "Error betting!." , "error")
    end
end)]]
        ...
        if not blackjackGameInProgress[gameId] then
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                        ...
                        if casinochips ~= nil and casinochips.amount >= betAmount then
                            if tryTakeChips(source,betAmount) then
                                --print("Taken",betAmount,"chips from id",source)
                                if blackjackGameData[gameId][source] == nil then
                                    blackjackGameData[gameId][source] = {}
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    TriggerClientEvent('CTRPFW:Notify', source,"- " ..tostring(lostAmount).." chips" , "error")
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent('CTRPFW:Notify', source, "- " ..tostring(potentialWinAmount).." chips" , "success")
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent('CTRPFW:Notify', source, "+0 chips" , "success")
                                                        end
                                                    end
                                                else
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent('CTRPFW:Notify', source, "- " ..tostring(potentialPushAmount).." chips" , "success")
                                                            if potentialPushAmount > 10000000 then

...
RegisterCommand("carry",function(source, args)
	if CTRPFW.Functions.GetPlayerData().metadata["isdead"] or CTRPFW.Functions.GetPlayerData().metadata["inlaststand"] then
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsControlJustPressed(1, 323) then --Start holding X
            if not handsup then
                TaskPlayAnim(GetPlayerPed(-1), dict, "handsup_enter", 2.0, 2.0, -1, 50, 0, false, false, false)
                handsup = true
            else
                handsup = false

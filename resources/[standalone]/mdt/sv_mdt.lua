...
--[[RegisterCommand('mdtv2', function(source, args, rawcmd)
	local src = source
...
AddEventHandler("mdt:performOffenderSearch", function(query)
	...
	CTRPFW.Functions.ExecuteSql(false, 'SELECT * FROM `players` WHERE `citizenid` = "'..query..'" OR `charinfo` LIKE "%'..query..'%"', function(result)
...
AddEventHandler('mdt:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('CTRPFW:Notify', OtherPlayer.PlayerData.source, "You received a fine of $"..price)
        end
    end
end)
...
AddEventHandler("mdt:server:getPlayer", function(playerid)
	...
	if playerid ~= nil then 
		_citizenID = CTRPFW.Functions.GetPlayer(playerid)
	end
...
function GetLicenses(identifier, cb)
	local Player = CTRPFW.Functions.GetPlayer(identifier)

...
AddEventHandler('police:server:CheckBills', function()
    ...
    CTRPFW.Functions.ExecuteSql(false, "SELECT * FROM `bills` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `type` = 'police'", function(result)
        if result[1] ~= nil then
            ...
            CTRPFW.Functions.ExecuteSql(false, "DELETE FROM `bills` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `type` = 'police'")
            TriggerClientEvent('police:client:sendBillingMail', src, totalAmount)
            --TriggerEvent('ctrp-moneysafe:server:DepositMoney', "police", totalAmount, "bills")
            TriggerClientEvent('ctrp-moneysafepolice:client:DepositMoney',"police", totalAmount, "bills")
		end
	end)
end)
...
AddEventHandler('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    local CuffedPlayer = CTRPFW.Functions.GetPlayer(playerId)
...
RegisterServerEvent('police:server:unnCuffPlayer')
AddEventHandler('police:server:unnCuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(source)
    local CuffedPlayer = CTRPFW.Functions.GetPlayer(playerId)
    if CuffedPlayer ~= nil then
...
AddEventHandler('police:server:EscortPlayer', function(playerId)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(source)
    local EscortPlayer = CTRPFW.Functions.GetPlayer(playerId)
    if EscortPlayer ~= nil then
        if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "doctor" ) or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
AddEventHandler('police:server:BillPlayer', function(playerId, amount, fname, lname, faname, laname)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
    ...
    local faname = OtherPlayer.PlayerData.charinfo.firstname
    local laname = OtherPlayer.PlayerData.charinfo.lastname

    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            --OtherPlayer.Functions.RemoveMoney("bank", amount, "paid-bills")
            TriggerClientEvent('CTRPFW:Notify', src, "You sent fine of $"..amount.. " to "..faname.." " ..laname.. " " )
            TriggerClientEvent('CTRPFW:Notify', OtherPlayer.PlayerData.source, "You received a fine of $"..amount.. " from "..fname.." " ..lname.. " " )
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('CTRPFW:Notify', src, "You sent the person to prison for "..time.." months")
        end
    end
end)

-- CTRPFW.Commands.Add("liveryp", "Livery", {{name="livery", help="Number of the livery"}}, true, function(source, args)
--     local Player = CTRPFW.Functions.GetPlayer(source)
...
AddEventHandler('police:server:SendEmergencyMessage', function(coords, message)
    local src = source
    local MainPlayer = CTRPFW.Functions.GetPlayer(src)
    local alertData = {
...
AddEventHandler('police:server:SearchPlayer', function(playerId)
    local src = source
    local SearchedPlayer = CTRPFW.Functions.GetPlayer(playerId)
    if SearchedPlayer ~= nil then
        TriggerClientEvent('chatMessage', source, "SYSTEM", "warning", "Person has $"..SearchedPlayer.PlayerData.money["cash"]..",- on him..")
        TriggerClientEvent('CTRPFW:Notify', SearchedPlayer.PlayerData.source, "You are being searched..")
    end
end)
...
AddEventHandler('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, CTRPFW.Shared.Items["moneybag"], "add")
        TriggerClientEvent('CTRPFW:Notify', SearchedPlayer.PlayerData.source, "Your cash is confiscated..")
    end
end)
...
AddEventHandler('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('CTRPFW:Notify', SearchedPlayer.PlayerData.source, "Your driving license has been confiscated..")
        else
            TriggerClientEvent('CTRPFW:Notify', src, "Can't confiscate driving license..", "error")
        end
    end
end)
...
AddEventHandler('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        TriggerClientEvent('CTRPFW:Notify', SearchedPlayer.PlayerData.source, "You have been robbed of $"..money.."..")
    end
end)
...
AddEventHandler('police:server:Impound', function(plate, fullImpound, price)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('CTRPFW:Notify', src, "Vehicle taken into depot for $"..price.."!")
        else
            exports['ghmattimysql']:execute('UPDATE player_vehicles SET state = @state WHERE plate = @plate', {['@state'] = 2, ['@plate'] = plate})
            TriggerClientEvent('CTRPFW:Notify', src, "Vehicle completely seized!")
        end
    end
end)
...
AddEventHandler('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        if Player.Functions.AddItem("filled_evidence_bag", 1, false, bloodInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, CTRPFW.Shared.Items["filled_evidence_bag"], "add")
            TriggerClientEvent("evidence:client:RemoveBlooddrop", -1, bloodId)
            BloodDrops[bloodId] = nil
        end
    else
        TriggerClientEvent('CTRPFW:Notify', src, "You must have an empty evidence bag with you", "error")
    end
end)
...
AddEventHandler('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local weaponInfo = CTRPFW.Shared.Weapons[weapon]
...
AddEventHandler('police:server:showFingerprintId', function(sessionId, fname, lname)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
...
AddEventHandler('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('CTRPFW:Notify', targetId, 'Your anklet is taken off.', 'error', 5000)
        TriggerClientEvent('CTRPFW:Notify', source, 'You took off an ankle bracelet from '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('CTRPFW:Notify', targetId, 'You put on an ankle strap.', 'error', 5000)
        TriggerClientEvent('CTRPFW:Notify', source, 'You put on an ankle strap to '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
...
AddEventHandler('police:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("ctrp-phone:client:addPoliceAlert", requestId, alertData)
end)
...
AddEventHandler('police:server:gunhiestalert', function(streetLabel, coords)
	...
	local Player = CTRPFW.Functions.GetPlayer(src)
    local alertData = {
        title = "Weapon Dealing",
        coords = {x = coords.x, y = coords.y, z = coords.z},
        description = "Weapon Dealing at "..streetLabel,
    }
    TriggerClientEvent("police:client:gunhiestalert", -1, streetLabel, coords)
    TriggerClientEvent("ctrp-phone:client:addPoliceAlert", -1, alertData)
    if Player.PlayerData.job.name == "police" then
        TriggerClientEvent('chatMessage', src, "SYSTEM", "error", "Gun Dealing In Progress")
    end
end)

CTRPFW.Functions.CreateCallback('police:server:isPlayerDead', function(source, cb, playerId)
    local Player = CTRPFW.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

CTRPFW.Functions.CreateCallback('police:GetPlayerStatus', function(source, cb, playerId)
    local Player = CTRPFW.Functions.GetPlayer(playerId)
    local statList = {}
	if Player ~= nil then
...
CTRPFW.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = CTRPFW.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(CTRPFW.Shared.Weapons[weapon]["name"])
    local retval = false
...
CTRPFW.Functions.CreateCallback('police:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    for k, v in pairs(CTRPFW.Functions.GetPlayers()) do
        local Player = CTRPFW.Functions.GetPlayer(v)
        if Player ~= nil then
...
CTRPFW.Functions.CreateCallback('police:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
CTRPFW.Functions.CreateCallback('police:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
CTRPFW.Functions.CreateCallback('police:GetCops', function(source, cb)
    local amount = 0
...
CTRPFW.Commands.Add("setpolice", "Give the police job to somebody ", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(tonumber(args[1]))
    local Myself = CTRPFW.Functions.GetPlayer(source)
    if Player ~= nil then 
        if Myself.PlayerData.job.name == "police" and Myself.PlayerData.job.gradelabel == "Commissioner" or Myself.PlayerData.job.gradelabel == "Chief" or Myself.PlayerData.job.gradelabel == "Deputy Commissioner" or Myself.PlayerData.job.gradelabel == "Assistant Commissioner" or Myself.PlayerData.job.gradelabel == "Captain" then
            Player.Functions.SetJob("police", tonumber(args[2]))
        else
			TriggerClientEvent('chatMessage', source, "Console", "error", "You are not allowed")
		end
	else
...
CTRPFW.Commands.Add("firepolice", "Fire a police officer!", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(tonumber(args[1]))
    local Myself = CTRPFW.Functions.GetPlayer(source)
    if Player ~= nil then 
        if Myself.PlayerData.job.name == "police" and Myself.PlayerData.job.gradelabel == "Commissioner" or Myself.PlayerData.job.gradelabel == "Chief" or Myself.PlayerData.job.gradelabel == "Deputy Commissioner" or Myself.PlayerData.job.gradelabel == "Assistant Commissioner" or Myself.PlayerData.job.gradelabel == "Captain" then
...
CTRPFW.Commands.Add("spikestrip", "Place a spike strip", {}, false, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    if Player ~= nil then
        if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
...
CTRPFW.Commands.Add("pobject", "Place/Delete an object", {{name="type", help="Type object you want or 'delete' to delete"}}, true, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
...
CTRPFW.Commands.Add("boombox", "Place/Delete an boombox", {{name="action", help="type1/type2/remove"}}, true, function(source, args)
...
CTRPFW.Commands.Add("cuff", "Cuff a player", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or (Player.PlayerData.job.name == "government" and Player.PlayerData.job.gradelabel == "Security Chief" or Player.PlayerData.job.gradelabel == "State Security") then
...
CTRPFW.Commands.Add("uncuff", "uncuff a player", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or (Player.PlayerData.job.name == "government" and Player.PlayerData.job.gradelabel == "Security Chief" or Player.PlayerData.job.gradelabel == "State Security") then
...
CTRPFW.Commands.Add("palert", "Make a police alert", {{name="alert", help="The Police alert"}}, false, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        if args[1] ~= nil then
            ...
            TriggerEvent("ctrp-log:server:CreateLog", "palert", "Police alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..msg, false)
...
CTRPFW.Commands.Add("escort", "Escort a person", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    TriggerClientEvent("police:client:EscortPlayer", source)
end)

--[[CTRPFW.Commands.Add("mdt", "Toggle police mdt", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
CTRPFW.Commands.Add("edmtablet", "Toggle Edm Tablet", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
...
CTRPFW.Commands.Add("callsign", "Put the name of your callsign (call number)", {{name="name", help="Name of your callsign"}}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    Player.Functions.SetMetaData("callsign", table.concat(args, " "))
end)

CTRPFW.Commands.Add("clearcasings", "Take away bullet casings in the area (make sure you have picked up some)", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
CTRPFW.Commands.Add("jail", "Jail a person", {{name="id", help="Player ID"},{name="time", help="Time they have to be in jail"}}, true, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
CTRPFW.Commands.Add("unjail", "Unjail a person.", {{name="id", help="Player ID"}}, true, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        local playerId = tonumber(args[1])
...
CTRPFW.Commands.Add("gimme", ":)", {}, false, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    local info = {
...
CTRPFW.Commands.Add("gimme2", ":)", {}, false, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    local info = {
        serie = "K"..math.random(10,99).."SH"..math.random(100,999).."HJ"..math.random(1,9),
        attachments = {
...
CTRPFW.Commands.Add("clearblood", "Take away nearby blood (make sure you've picked up some)", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
CTRPFW.Commands.Add("seizecash", "Take cash from the nearest person", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
CTRPFW.Commands.Add("sc", "Handcuff someone but he can walk", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "government" and Player.PlayerData.job.gradelabel == "Security Chief" or Player.PlayerData.job.gradelabel == "State Security" then
...
CTRPFW.Commands.Add("cam", "View security camera", {{name="camid", help="Camera ID"}}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
CTRPFW.Commands.Add("flagplate", "Flag a vehicle", {{name="plate", help="License"}, {name="reason", help="Reason of flagging the vehicle"}}, true, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('CTRPFW:Notify', source, "Vehicle ("..args[1]:upper()..") is flagged for: "..table.concat(reason, " "))
    else
        TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "This command is for emergency services!")
    end
end)

CTRPFW.Commands.Add("unflagplate", "Unflag a vehicle", {{name="plate", help="License plate"}}, true, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
                Plates[args[1]:upper()].isflagged = false
                TriggerClientEvent('CTRPFW:Notify', source, "Vehicle ("..args[1]:upper()..") is unflagged")
            else
...
CTRPFW.Commands.Add("plateinfo", "Flag a vehicle", {{name="plate", help="License plate"}}, true, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
...
CTRPFW.Commands.Add("depot", "Send a vehicle to the depot", {{name="price", help="Price for how much the person has to pay (may be empty)"}}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "doctor"  then
...
CTRPFW.Commands.Add("seize", "Seize a vehicle", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "doctor"  then
...
CTRPFW.Commands.Add("impound", "Impound a vehicle", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "doctor" or Player.PlayerData.job.name == "pdm" or Player.PlayerData.job.name == "edm" or Player.PlayerData.job.name == 'mechanic' then
...
CTRPFW.Commands.Add("paytow", "Pay a bergnet worker", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "doctor" then
        local playerId = tonumber(args[1])
        local OtherPlayer = CTRPFW.Functions.GetPlayer(playerId)
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('CTRPFW:Notify', source, 'You paid a bergnet worker')
            else
                TriggerClientEvent('CTRPFW:Notify', source, 'Person is not a bergnet worker', "error")
            end
        end
    else
        TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "This command is for emergency services!")
...
CTRPFW.Commands.Add("paylaw", "Pay a lawyer", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or (Player.PlayerData.job.name == "doj" and Player.PlayerData.job.gradelabel == "Judge" or Player.PlayerData.job.gradelabel == "Chief Justice") then
        local playerId = tonumber(args[1])
        local OtherPlayer = CTRPFW.Functions.GetPlayer(playerId)
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "lawyer" then
                ...
                TriggerClientEvent('CTRPFW:Notify', source, 'You paid a lawyer')
            else
                TriggerClientEvent('CTRPFW:Notify', source, 'Person is not a lawyer', "error")
            end
        end
    else
        TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "This command is for emergency services!")
...
CTRPFW.Commands.Add("radar", "Toggle speedradar :)", {}, false, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
--[[CTRPFW.Functions.CreateUseableItem("handcuffs", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
...
CTRPFW.Functions.CreateUseableItem("handcuffs", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("police:client:CuffPlayerSofts", source)
    end
end)

CTRPFW.Functions.CreateUseableItem("keyz", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
...
CTRPFW.Commands.Add("911", "Send a report to emergency services", {{name="message", help="Message you want to send"}}, true, function(source, args)
    ...
    if Player.Functions.GetItemByName("phone") ~= nil then
        ...
        TriggerEvent("ctrp-log:server:CreateLog", "911", "911 alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..message, false)
    else
        TriggerClientEvent('CTRPFW:Notify', source, 'You dont have a phone', 'error')
    end
end)

CTRPFW.Commands.Add("911a", "Send an anonymous report to emergency services (gives no location)", {{name="message", help="Message you want to send"}}, true, function(source, args)
    local message = table.concat(args, " ")
    local Player = CTRPFW.Functions.GetPlayer(source)
...
CTRPFW.Commands.Add("911r", "Send a message back to a alert", {{name="id", help="ID of the alert"}, {name="Message", help="Message you want to send"}}, true, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    local OtherPlayer = CTRPFW.Functions.GetPlayer(tonumber(args[1]))
    table.remove(args, 1)
    local message = table.concat(args, " ")
    local Prefix = "POLICE"
...
CTRPFW.Commands.Add("311", "Send a report to emergency services", {{name="message", help="Message you want to send"}}, true, function(source, args)
    local message = table.concat(args, " ")
    local Player = CTRPFW.Functions.GetPlayer(source)

    if Player.Functions.GetItemByName("phone") ~= nil then
        TriggerClientEvent("police:client:SendEmergencyMessages", source, message)
        TriggerEvent("ctrp-log:server:CreateLog", "311", "311 alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..message, false)
    else
        TriggerClientEvent('CTRPFW:Notify', source, 'You dont have a phone', 'error')
    end
end)

CTRPFW.Commands.Add("311a", "Send an anonymous report to emergency services (gives no location)", {{name="message", help="Message you want to send"}}, true, function(source, args)
    local message = table.concat(args, " ")
    local Player = CTRPFW.Functions.GetPlayer(source)
...
CTRPFW.Commands.Add("311r", "Send a message back to a alert", {{name="id", help="ID of the alert"}, {name="Message", help="Message you want to send"}}, true, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    local OtherPlayer = CTRPFW.Functions.GetPlayer(tonumber(args[1]))
    table.remove(args, 1)
    local message = table.concat(args, " ")
...
CTRPFW.Commands.Add("anklet", "Put an anklet on the nearest person.", {}, false, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
...
CTRPFW.Commands.Add("ankletlocation", "remove an anklet off the nearest person", {{"bsn", "BSN of person"}}, true, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)

    if Player.PlayerData.job.name == "police" then
        if args[1] ~= nil then
            local citizenid = args[1]
            local Target = CTRPFW.Functions.GetPlayerByCitizenId(citizenid)
            ...
            if Target ~= nil then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('CTRPFW:Notify', source, 'This person does not have an anklet.', 'error')
                end
            end
        end
    else
...
CTRPFW.Commands.Add("ebutton", "Send a message back to a notification", {}, false, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "doctor" ) and Player.PlayerData.job.onduty) then
...
CTRPFW.Commands.Add("takedrivinglicense", "Take the driving license from someone", {}, false, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) then
...
CTRPFW.Commands.Add("fine", "Give Fine To Person", {{name="id", help="Player ID"},{name="amount", help="Amount"}}, true, function(source, args)
	local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        local playerId = tonumber(args[1])
        ...
        if amount > 0 then
            ...
            TriggerClientEvent('ctrp-moneysafepolice:client:DepositMoney', playerId , amount)
        else
...
CTRPFW.Commands.Add("takedna", "Take a DNA sample from a person (empty evidence bag needed)", {{"id", "ID of the person"}}, true, function(source, args)
    local Player = CTRPFW.Functions.GetPlayer(source)
    local OtherPlayer = CTRPFW.Functions.GetPlayer(tonumber(args[1]))
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            if Player.Functions.AddItem("filled_evidence_bag", 1, false, info) then
                TriggerClientEvent("inventory:client:ItemBox", source, CTRPFW.Shared.Items["filled_evidence_bag"], "add")
            end
        else
            TriggerClientEvent('CTRPFW:Notify', source, "You must have an empty evidence bag with you", "error")
        end
    end
end)

CTRPFW.Functions.CreateUseableItem("moneybag", function(source, item)
    local Player = CTRPFW.Functions.GetPlayer(source)
    if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        if item.info ~= nil and item.info ~= "" then
            if Player.PlayerData.job.name == "police" and Player.PlayerData.job.gradelabel == "Chief" or Myself.PlayerData.job.gradelabel == "Commissioner" then
...
CTRPFW.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    local retval = false
    for k, v in pairs(CTRPFW.Functions.GetPlayers()) do
        local Player = CTRPFW.Functions.GetPlayer(v)
        if Player ~= nil then
...
CTRPFW.Commands.Add('livery', 'Set vehicle livery (Emergency Only)', {}, false, function(source, args)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
...
CTRPFW.Commands.Add('extras', 'Set vehicle Extras (Emergency Only)', {{name="extra", help="all / remove / 1-12"}}, false, function(source, args)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
...
CTRPFW.Commands.Add('pfix', 'Fix Vehicle (Emergency Only)', {}, false, function(source, args)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
...
CTRPFW.Commands.Add('ptint', 'Set Tint (Emergency Only)', {}, false, function(source, args)
    local src = source
    local Player = CTRPFW.Functions.GetPlayer(src)
    if Player and (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "doctor" ) then

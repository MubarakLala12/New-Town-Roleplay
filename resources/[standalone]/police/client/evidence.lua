...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	CTRPFW.Functions.Progressbar("clear_blooddrops", "Clearing blood..", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then
			...
			CTRPFW.Functions.Notify("Blood cleared :)")
		end
    end, function() -- Cancel
        CTRPFW.Functions.Notify("Blood not cleared..", "error")
    end)
end)
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(PlayerPedId())
	local casingList = {}
	CTRPFW.Functions.Progressbar("clear_casings", "Removing bullet sleeves..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then
			...
			CTRPFW.Functions.Notify("Bullet sleeves removed :)")
		end
    end, function() -- Cancel
        CTRPFW.Functions.Notify("Bullet sleeves not removed", "error")
    end)
end)
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, true) < 1.5 then
				...
				if IsControlJustReleased(0, Keys["G"]) then
					...
					local info = {
						...
						ammolabel = Config.AmmoLabels[CTRPFW.Shared.Weapons[Casings[CurrentCasing].type]["ammotype"]],
		...
		if isArmed then
			if PlayerJob.name == 'police' and onDuty then
				if IsPedShooting(PlayerPedId()) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
					...
					if CTRPFW.Shared.Weapons[currentWeapon]["ammotype"] ~= "AMMO_PISTOL" then
						automatic = true
					end
...
function IsSilentWeapon(weapon)
	...
	if not retval then
		CTRPFW.Functions.TriggerCallback('police:IsSilencedWeapon', function(result)
			retval = result
			return result
		end, weapon)
		Citizen.Wait(100)

        ...
        if ClosestSafe ~= nil then
            if PlayerData.job.name == ClosestSafe then
                if IsAuthorized then
                    ...
                    if PlayerJob.name == 'police' and PlayerJob.gradelabel == "Chief" then
                        if distance < 20 then
                            inRange = true
                            if distance < Config.MinimumSafeDistance then
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '~g~$'..data.money)
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z - 0.1, '~b~/add~w~ - Put money in the safe')
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z - 0.2, '~b~/with~w~ - Take money out of the safe')
                            end
                        end
                    end
                else
...
RegisterNetEvent('ctrp-moneysafepolice:client:DepositMoney')
AddEventHandler('ctrp-moneysafepolice:client:DepositMoney', function(amount)
    if ClosestSafe ~= nil then
        if IsAuthorized then
            local ped = PlayerPedId()
            local pos = GetEntityCoords(ped)
            ...
            TriggerServerEvent('ctrp-moneysafepolice:server:DepositMoney', ClosestSafe, amount)
        end
    end
end)

RegisterNetEvent('ctrp-moneysafepolice:client:WithdrawMoney')
AddEventHandler('ctrp-moneysafepolice:client:WithdrawMoney', function(amount)
    if ClosestSafe ~= nil then
        if IsAuthorized then
            local ped = PlayerPedId()
            local pos = GetEntityCoords(ped)
            ...
            if distance < Config.MinimumSafeDistance then
                TriggerServerEvent('ctrp-moneysafepolice:server:WithdrawMoney', ClosestSafe, amount)
            end
        end
    end
end)
...
RegisterNetEvent('ctrp-moneysafepolice:client:UpdateSafe')
AddEventHandler('ctrp-moneysafepolice:client:UpdateSafe', function(SafeData, k)

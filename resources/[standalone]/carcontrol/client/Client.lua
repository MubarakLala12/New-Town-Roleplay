...
ctrl.update = function()    
    Wait(1000)    
    SendNUIMessage({var='onChange',value='https://carcontrol/posted'})     
    while true do      
...
ctrl.setSeat = function(d,s)   
    ...
    lastData = gotOptions   
    gotOptions = false   
    SendNUIMessage({var='options',ret='https://carcontrol/gotOpts'})   
...
ctrl.setEngine = function(d,s)
    ...
    if GetIsVehicleEngineRunning(v) then
        --exports['mythic_notify']:SendAlert('info', 'Engine Off', 2500)
        CTRPFW.Functions.Notify("Engine Off", "error")
        EngineControl()
    else
        --exports['mythic_notify']:SendAlert('info', 'Engine On', 2500)
        CTRPFW.Functions.Notify("Engine On", "success")
        EngineControl()
    end
end  

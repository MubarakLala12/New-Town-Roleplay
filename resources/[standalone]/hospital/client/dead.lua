		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then
                    if deathTime > 0 then
                        DrawTxt(0.93, 1.44, 1.0,1.0,0.6, "RESPAWN OVER: ~r~" .. math.ceil(deathTime) .. "~w~ SECONDS", 255, 255, 255, 255)
                    else
                        DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, "~w~ Hold ~r~[E] ("..hold..")~w~ PRESSED TO RESPAWN ~r~($"..Config.BillCost..")~w~", 255, 255, 255, 255)
                    end
                end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, "YOUR BLOOD OUT OVER: ~r~" .. math.ceil(LaststandTime) .. "~w~ SECONDS", 255, 255, 255, 255)
                else
                    DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, "YOUR BLOOD OUT OVER: ~r~" .. math.ceil(LaststandTime) .. "~w~ SECONDS, YOU CAN BE TREATED", 255, 255, 255, 255)
                end

...
local zoomspeed = 7.0 -- camera zoom speed
...
local toggle_spotlight = 304 -- control id to toggle the front spotlight Default: INPUT_PhoneCameraGrid (G)
...
function HandleSpotlight(cam)
    ...
    if spotlight_state then
        ...
        DrawSpotLight(camcoords, forward_vector, 255, 255, 255, 2000.0, 0.5, 0.5,
                      7.50, 75.0)
            ...
            for key, value in pairs(spotlights) do
                if value.enabled and key ~= currentPlayerId and value.camcoords ~=
                    ...
                    DrawSpotLight(value.camcoords, value.forward_vector, 255,
                                  255, 255, 2000.0, 0.5, 0.5, 7.50, 75.0)
                end
            end
        end
        Citizen.Wait(0)
